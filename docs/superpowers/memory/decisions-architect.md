...
**Decision:** Replace the DDD plugin's single `standards` skill with `design`, `implement`, and `review`. All three skills share plugin-root `references/` and read `ddd-risk-router.md` first, while hooks map planning to design guidance, execution to implementation guardrails, and code-review to boundary review. The phase skills own thinking frameworks: design owns Product semantics intake and Spec trace; implement owns Design input check, Model-to-code placement, and Implementation trace; review owns Evidence map, Expected model vs observed code, and Finding triage. Design's default prompt-time budget is risk-router/modeling/core, with database on-demand for explicit persistence concerns.
