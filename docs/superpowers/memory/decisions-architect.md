...
**Decision:** Replace the DDD plugin's single `standards` skill with `design`, `implement`, and `review`. All three skills share plugin-root `references/` and read `ddd-risk-router.md` first, while hooks map planning to design guidance, execution to implementation guardrails, and code-review to boundary review. The design skill owns Product semantics intake, Spec trace, command/query/event/lifecycle modeling, and stop questions; its default prompt-time budget is risk-router/modeling/core, with database on-demand for explicit persistence concerns.
**Trade-off:** The plugin exposes more skills and gives design a stronger product-semantics contract, but each entry point is simpler and has a clearer action/phase output. Accepted because one broad standards entry left agents to infer whether they were designing, placing code, or auditing a diff, and the earlier design workflow risked becoming a passive reference-reading checklist.
