...
`superpowers-ddd-architect` originally provided DDD/backend phase skills plus Claude/Codex hook routing for Superpowers workflows. The DDD guidance now needs to work in hookless and non-Superpowers skill systems. Keeping both `superpowers-ddd-architect` and standalone `ddd-expert` would duplicate the same DDD reference set and phase skills, creating drift risk and an unclear canonical entry point.
...
Make `ddd-expert` the canonical DDD/backend guidance plugin. It remains explicit and hookless, with `design`, `implement`, and `review` phase skills plus shared DDD/backend references under plugin-root `references/`.
...
Strengthen `ddd-expert:review` with an Evidence Gate that classifies touched surfaces from concrete evidence, routes each surface to required references/evidence, reports Rules Satisfied / Not Applicable / Exception, and treats missing proof as an evidence gap rather than a finding.
...
- `ddd-expert` can be used by Superpowers and non-Superpowers skill systems without hook assumptions.
