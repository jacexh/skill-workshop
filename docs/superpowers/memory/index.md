...
- [architecture.md](architecture.md) — System boundaries, components, data flows
  Key points: dual-track marketplace; DDD architect has design/implement/review skills and owns all bundled DDD references; old architect is explicit-only with no bundled DDD/database files; memory SessionStart reports KB availability/status only; Codex designing-tests is UserPromptSubmit-only
...
- [features.md](features.md) — Current capability map
  Key points: DDD architect guidance uses phase-specific skills over one risk-router first read for Go/Python/TypeScript backend DDD; memory query reads index on demand and reports retrieval routes; ingest can rebuild decision/glossary roots into router + shard layouts
