...
**Enables** — Agents can apply focused DDD/backend guidance from evidence-first strategic domain modeling and event storming through modeling gates, product-semantic design, model-to-code implementation, workflow-based evidence review, Go events/messages, taskqueue/runtime, and on-demand database-backed persistence.

**Actors / Entry Points** — `$ddd-expert:domain-modeling`, `$ddd-expert:design`, `$ddd-expert:implement`, `$ddd-expert:review`, and shared reference files under both `plugins/ddd-expert/references/` and `codex-plugins/ddd-expert/references/`.

**Capability Boundary** — The phase skills own entry timing, slim output contracts, and reference routing. `$ddd-expert:domain-modeling` owns the strategic interview before design: it inspects existing sources before asking, reconstructs user scenarios, walks event-storming timelines in business language, explores authority/policy/hotspot questions one at a time, keeps replies short, and synthesizes conclusions only after a short question chain; tactical Aggregate/Repository/Event consequences are deferred to the DDD handoff. `ddd-modeling-gates.md` is the compact thinking spine for story, authority, lifecycle, invariants, failure tolerance, integration language, and coordination choices before tactical DDD objects are named. `$ddd-expert:design` answers relevant gates before Aggregates, Repositories, ports, handlers, schemas, transactions, or event payloads; its handoff records which modeling decisions justify tactical objects. `$ddd-expert:implement` consumes that handoff, verifies modeling evidence when boundaries matter, and stops back to design when synchronous writes across candidate Aggregate Roots lack invariant/failure-tolerance evidence. `$ddd-expert:review` reconstructs expected model evidence from the Domain Modeling Brief, design, Implementation handoff, spec/ADR/code/tests, then uses its Workflow and Layer Baseline to produce smell rows, merged smell families, closed verdicts, root synthesis, findings, evidence gaps, or no-finding notes with explicit observed-correct-shape admission. `ddd-core.md` owns compact cross-phase DDD defaults and evidence rules; strategic/language/runtime/database references load only when the phase skill or touched evidence needs them. The plugin is explicit; hooks are restrained route reminders for upstream Superpowers workflow mentions, so non-Superpowers systems can still use the phase skills directly.

**References** — `plugins/ddd-expert/skills/domain-modeling/SKILL.md`, `plugins/ddd-expert/skills/design/SKILL.md`, `plugins/ddd-expert/skills/implement/SKILL.md`, `plugins/ddd-expert/skills/review/SKILL.md`, `plugins/ddd-expert/references/ddd-modeling-gates.md`, `plugins/ddd-expert/references/ddd-modeling.md`, `plugins/ddd-expert/references/ddd-core.md`, `plugins/ddd-expert/references/ddd-golang.md`, `plugins/ddd-expert/references/ddd-python.md`, `plugins/ddd-expert/references/ddd-typescript.md`, `plugins/ddd-expert/references/ddd-golang-events-messages.md`, `plugins/ddd-expert/references/ddd-golang-runtime.md`, `plugins/ddd-expert/references/ddd-golang-taskqueue.md`, `plugins/ddd-expert/references/database.md`; see `conventions.md` for reference parity rules, ADR-025 for the standalone plugin decision, ADR-022 for phase-specific skills, ADR-015 for the agent-contract/runtime split, ADR-017 for the taskqueue split, ADR-018 for the events/messages split, and `docs/superpowers/specs/2026-07-07-ddd-expert-modeling-gates-design.md` for the modeling-gates design.

#### DDD Code Agent Execution Contract

**Enables** — Code agents (Claude Code, Codex) can load a deterministic self-check contract when DDD, Go events/messages, Go runtime, or Go taskqueue work needs task classification, stop-and-ask protocol, prohibited actions, matching self-checks, or cross-phase reporting facts.

**Actors / Entry Points** — `plugins/ddd-expert/references/ddd-agent-contract.md` (and its Codex mirror); referenced from DDD phase skills and loaded only when task classification, prohibited actions, stop protocol, or self-checks are needed.
