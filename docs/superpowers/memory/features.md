...
**Enables** — Agents can apply focused DDD/backend guidance from product-semantic design through risk routing, strategic modeling, tactical implementation, Go events/messages, taskqueue/runtime, and on-demand database-backed persistence.
...
**Capability Boundary** — Hooks inject compact DDD Design Guidance, DDD Implementation Guardrails, or DDD Boundary Review only when selected upstream workflow/review skills are invoked. Design hook guidance names Product semantics intake, Spec trace, and command/query/event/lifecycle modeling but leaves the full method in the `design` skill. Agents must read `ddd-risk-router.md` first, then deeper references only when a risk card, task, or gate requires them.
