...
**Enables** — Agents can apply focused DDD/backend guidance from evidence-first strategic domain modeling and event storming through product-semantic design, model-to-code implementation, evidence-based review, risk routing, Go events/messages, taskqueue/runtime, and on-demand database-backed persistence.
...
**Capability Boundary** — The phase skills own entry timing, slim output contracts, and reference routing. `$ddd-expert:domain-modeling` owns the strategic interview before design: it inspects existing sources before asking, reconstructs user scenarios, walks event-storming timelines in business language, explores authority/policy/hotspot questions one at a time, keeps replies short, and synthesizes conclusions only after a short question chain; tactical Aggregate/Repository/Event consequences are deferred to the DDD handoff. `$ddd-expert:design` turns an accepted Domain Modeling Brief into implementable DDD/backend design: it confirms strategic facts, decides only tactical mechanisms implementation will depend on, names testing seams, and emits an Implementation handoff covering accepted source, capability/context/language, data authority, aggregate-or-policy boundary, commands/queries/events/messages/reactions, consistency/failure rules, and layer ownership; missing material handoff facts are stop conditions. `$ddd-expert:implement` consumes that handoff, runs a handoff check, object-shape routing, evidence-driven surface preflight, placement translation, rule-status reporting, and verification without inventing domain facts. `$ddd-expert:review` reconstructs expected model sources from the Domain Modeling Brief, design, Implementation handoff, spec/ADR/code/tests, then runs an evidence gate across domain/spec/code axes and reports findings, allowed exceptions, harmless local style, or evidence gaps. `ddd-risk-router.md` and deeper references own detailed risk cards, required evidence, allowed exceptions, language/runtime/database rules, and Go-specific object cards; phase skills stay concise and load those references on demand. The plugin is explicit and hookless so it can be used by non-Superpowers skill systems.
