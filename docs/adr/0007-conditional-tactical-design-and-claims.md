...
EventStorming normalizes Commands by the intent chosen by the actor or issuing policy, then projects **Aggregate Capabilities** and only business-required **Reactions** into each affected canonical Model. Every in-scope state-changing Command maps to one or more capabilities or explicit coordination, and every new or changed capability names its source Command or Commands. One capability is one stable Root operation that guides its intention-revealing Domain method surface; pre-state, success, rejection, and timing branches remain scenario, lifecycle, invariant, or policy meaning rather than separate capability rows. Capabilities state only common required authoritative facts, guaranteed business result, and stable rejection. They prescribe neither signatures nor exact method counts, handlers, transport, transactions, retry mechanisms, packages, persistence, or Application orchestration.

EventStorming minutes retain the complete scenario diagram and one compact Command-to-Capability mapping, not a second copy of each full capability contract. The affected `model.md` is the sole current capability authority; every contract field projected there must already be visible in the confirmed scenario threads. A reaction records `selected Domain Event or Published Fact Contract -> Reaction Policy and owner -> Command -> Aggregate Capability or explicit coordination`, plus business failure/recovery meaning. Existing capability tables remain readable authority and migrate only when EventStorming next changes their Model; no format-only Model revision is created.
