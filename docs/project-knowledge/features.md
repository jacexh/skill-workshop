...
**Capability Boundary** — `ddd-modeling.md` owns architecture gates, model discovery, mandatory technical-capability-before-port classification, port granularity (including the capability-lifecycle unit and inward-port extension default in §0.2.1-§0.2.2), async handler cards, generalized routing/ownership examples covering Dispatcher-style systems, and vendor-wrapper ACL triage; `ddd-core.md` owns language-neutral DDD/Clean Architecture rules, CQRS product-read port boundaries, Domain Event vs Integration Message boundaries, and async reaction roles; `ddd-golang.md` covers Go layers/aggregates/repositories/CQRS/file layout/module assembly and treats named Go component libraries as the project-default stack, including transaction-shape guardrails, field-vs-method aggregate behavior guidance, and thin RPC shortcut rules; `ddd-golang-events-messages.md` carries Go events/messages concerns (event.Collection, Domain Event Handlers, Boundary Publishers, Integration Messages, message handlers, message.Subscriber registration, message.Runner runtime loops, Kafka FailurePolicy, adapter wiring, idempotency, failure semantics); `ddd-golang-runtime.md` carries Go runtime concerns (config, fx module assembly guardrails, thin `cmd`, named `internal/pkg/module.go` service modules, fx.Lifecycle, graceful shutdown, Kubernetes); `ddd-golang-taskqueue.md` carries Go taskqueue/polling/periodic concerns (TaskType, PeriodicTask, Schedule, schema registry, processors, asynq worker/scheduler wiring, middleware, polling policy, periodic schedule/policy validation, disabled-task registration, duplicate-name scope, business-visible scheduling placement); Python/TypeScript guides carry the same shared gates, placement rules, CQRS product-read boundaries, routing/topology port prohibitions, event-drain lifecycle, and cross-context message boundary in language-specific form.
