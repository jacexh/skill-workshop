...
- **DDD pattern ownership:** `ddd-modeling.md` owns strategic modeling, architecture gates, technical-capability classification, port granularity, and vendor-wrapper ACL triage; `ddd-core.md` owns language-neutral tactical rules, Domain Event vs Integration Message boundaries, generated protocol DTO boundaries, and review checklist; `ddd-<language>.md` files add implementation-specific placement, validation, testing, observability, wiring guidance, and language-specific dependency guidance while preserving shared DDD gates and boundaries. Go guidance standardizes Domain Events on `github.com/go-jimu/components/ddd/event`, Integration Message ports on `github.com/go-jimu/components/ddd/message`, and the default Kafka adapter on `github.com/go-jimu/contrib/message/kafka`; Python/TypeScript guidance mirrors the shared Go-era rules without copying Go-only runtime libraries.
...
- **DDD canonical Go component libraries:** `ddd-golang.md` treats its named Go libraries as requirements for their concerns, not suggestions. Agents must use the public interfaces from `ddd/event`, `ddd/message`, `contrib/message/kafka`, `fsm`, `sloghelper`, and `config` instead of inventing local substitutes unless existing repository code or explicit user direction establishes an exception.
- **DDD agent execution contract (ADR-015):** `ddd-agent-contract.md` is the agent-behavior layer for DDD work. Agents classify the task as DDD-business / Go-runtime-only / mixed; only DDD-business and mixed paths emit the modeling §0 Architecture Gate, runtime-only work declares runtime component/ownership/lifecycle/config/shutdown instead. The contract owns the 13 must-not rules, including the prohibition on inventing local substitutes for canonical Go component libraries, the stop-and-ask protocol (preceded by repo-context inspection), the dual-track self-check (DDD §5.1 + Runtime §5.2), and the compact output template; modeling/core/language docs do not duplicate these.
