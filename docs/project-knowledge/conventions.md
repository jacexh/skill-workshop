...
- **KB writes go through `superpowers-memory:update` / `superpowers-memory:rebuild` only** (ADR-010). The PreToolUse hook blocks `Write` / `Edit` / `MultiEdit` / `NotebookEdit` on any path under `docs/project-knowledge/` unless a write-lock (`.git/superpowers-memory.lock`, 60-min TTL) is held. There is no escape hatch — manual typo fixes also go through `superpowers-memory:update`.
