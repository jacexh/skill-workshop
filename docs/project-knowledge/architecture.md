...
| `codex-plugins/superpowers-architect/hooks/codex-runtime.js` | Modes: `session-start` emits standing pattern index; `user-prompt-submit` matches explicit upstream `superpowers` workflow skill mentions; `stop` blocks only obvious plan/implementation/review answers that omit an architecture-standards judgment. Pattern dirs resolve bundled, global, and project design-pattern directories with later dirs overriding by filename | Output via `hookSpecificOutput.additionalContext` or `decision: "block"` for Stop continuation | Node.js, git |
...
12. **Architect / test guidance on Codex:** architect uses SessionStart standing index plus UserPromptSubmit matching for explicit upstream `superpowers` skill mentions and `$superpowers-architect:standards`; its narrow Stop hook requests a one-time standards judgment only after obvious plan/implementation/review answers that omitted it. Designing-tests remains SessionStart-only with execution-tier principles + reference index. No implicit skill-call JIT is available on Codex (ADR-013, ADR-014).
