...
12. **Architect / test guidance on Codex:** architect uses SessionStart standing index plus UserPromptSubmit prompt heuristics and `$superpowers-architect:standards`; designing-tests remains SessionStart-only with execution-tier principles + reference index. No skill-call JIT available on Codex (ADR-013).
...
- **Codex marketplace compat — Strategy A:** Parallel `codex-plugins/` tree with full duplication; Codex track is experimental; setup installer handles strict-JSON `~/.codex/hooks.json` registration. Architect Codex now layers prompt-time heuristics and an explicit standards skill over the ADR-013 SessionStart fallback.
