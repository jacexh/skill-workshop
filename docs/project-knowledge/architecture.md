...
4. **PreToolUse interception (memory):** Claude Code fires `PreToolUse` → `hook-runtime.js pre-tool-use` parses stdin JSON and dispatches by `tool_name`. For `Skill`: per-skill dispatch via the `skillAdvisory` map. The 4 planning/execution skills get a 1-line advisory; `finishing-a-development-branch` uses a 4-way classifier — KB missing → `decision: block` (catastrophic); on base branch or detached HEAD → `{}` (no-op); KB does not cover HEAD → architect-style rich-context block from `buildFinishingRichContext()` carrying commits + files since `covers_branch@SHA` plus imperative "must invoke `superpowers-memory:update` next" (ADR-011); KB covers HEAD → soft reminder. For `Write` / `Edit` / `MultiEdit` / `NotebookEdit`: if `tool_input.file_path` resolves under `docs/project-knowledge/`, blocks unless `.git/superpowers-memory.lock` is held (60-min TTL; acquired/released only by `superpowers-memory:update` and `superpowers-memory:rebuild`) (ADR-010)
