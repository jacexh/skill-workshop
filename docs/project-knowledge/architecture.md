...
| `plugins/superpowers-memory/hooks/` | Hook scripts that inject behavior context into agent sessions | `hooks.json` declares event bindings; `run-hook.cmd` is the cross-platform dispatcher | bash, git |
...
2. **Session start:** Claude Code fires `SessionStart` hook → `run-hook.cmd session-start` executes → script checks for `docs/project-knowledge/` in the user's project → outputs JSON context injected into agent's session
...
4. **Task completion:** Claude Code fires `TaskCompleted` → `run-hook.cmd task-completed` → JSON reminder to update plan checkboxes injected into agent context
5. **Session end:** Claude Code fires `Stop` → `run-hook.cmd stop` → script checks `git diff` for plan file changes → conditionally injects reminder to run `:update`
