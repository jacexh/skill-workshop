...
| `codex-plugins/superpowers-architect/hooks/codex-runtime.js` | Modes: `session-start` emits standing pattern index + dynamic Architecture Gate; `user-prompt-submit` matches explicit upstream `superpowers` workflow skill mentions; legacy `stop` mode returns `{}` for older installed configs. Pattern dirs resolve bundled, global, and project design-pattern directories with later dirs overriding by filename | Output via `hookSpecificOutput.additionalContext`; no Stop hook is registered | Node.js, git |
| `codex-plugins/designing-tests/` | Codex track: architecture-aware test design principles + reference index | Skill (`designing-tests`, copied) + `cleanup` skill | Codex CLI plugin runtime, Node.js |
| `codex-plugins/designing-tests/hooks/codex-runtime.js` | Modes: `session-start` and `user-prompt-submit`; emits compact guidance for architecture docs, boundary selection, real/shallow/fake labels, integration reality, hand-off evidence, and reference paths; full SKILL.md available on demand via `$designing-tests:designing-tests` | UserPromptSubmit responds to explicit upstream `$superpowers:*` workflow skill mentions; natural language stays quiet | Node.js |
...
10. **PreToolUse interception (Codex memory):** matcher `apply_patch|mcp__filesystem__.*` → `resolveTargetPath()` extracts path from `tool_input.{file_path,path,patch}` → returns `hookSpecificOutput.permissionDecision = "deny"` when path is under `docs/project-knowledge/` and lock is not held (ADR-010 carries over; same lock file `.git/superpowers-memory.lock`).
11. **Skill interception (architect Claude / designing-tests Claude):** PreToolUse:Skill on trigger skills → injects pattern index or test-design tier text. Claude architect also exposes `$superpowers-architect:standards` for explicit standards workflow use outside automatic trigger paths.
12. **Architect / test guidance on Codex:** architect uses SessionStart dynamic pattern index + Architecture Gate, UserPromptSubmit matching for explicit upstream `superpowers` skill mentions, and `$superpowers-architect:standards`; it intentionally does not register a Stop hook because Codex Stop fires per assistant turn. Designing-tests uses SessionStart plus UserPromptSubmit matching for explicit upstream workflow skill mentions to reinforce architecture-aware test design and hand-off evidence. No implicit agent-self-decided skill-call JIT is available on Codex (ADR-013, ADR-014).
