...
Standalone hookless DDD/backend architecture expert skills for code agents. EventStorming is the single modeling workflow: it keeps discovery on a temporary EventStorming Board, runs the ten EventStorming steps through one frontier question at a time, adversarially reviews the complete integrated model, then writes incremented canonical `draft` Models as the approval surface. The console summarizes those files and requests confirmation; approval promotes the same revisions to `model_ready` and synchronizes the remaining project documents. That `model_ready` result enters Codify directly; Codify makes the engineering realization decisions, and Guard performs parallel Model Realization/House-Style Conformance review. Every material Codify change completes through a clear independent Guard in the same task.

Invoke EventStorming with a user story, business scenario, Spec, PRD, or existing Model. It proceeds from past-tense events through timeline, Commands, actors/external systems, rules, Hotspots, Aggregates/core objects, Bounded Contexts, and collaboration. The facilitator investigates available evidence, asks the user one material fact or decision at a time, recommends design choices with their strongest credible alternative, and allows later evidence to reopen working conclusions. Package names, services, tables, runtime components, and call direction are evidence rather than context boundaries. Repeated mechanisms are evaluated with DRY-as-knowledge, cohesion, information hiding, coupling, and YAGNI before choosing shared domain ownership, a shared technical Module, or distinct local semantics with translations. The Context Map records the acyclic semantic `U -> D` dependency graph and its named contracts; relevant cross-context scenario interactions remain in the EventStorming Models. Only explicit confirmation of the complete integrated model authorizes one synchronized documentation closure across relevant DDD, Spec, PRD, ADR, and Glossary artifacts. Written Models preserve the confirmed diagram and use `model_status: model_ready`, which is direct Codify authority.

In Codify, house-style guidance keeps Domain/Application/Transport/Infrastructure responsibilities explicit while project evidence determines Repository/CQRS shape, ports, persistence, adapters, runtime wiring, migrations, and verification. Guard independently checks both Model realization and conformance.
