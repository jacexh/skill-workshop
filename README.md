...
Standalone hookless DDD/backend architecture expert skills for code agents. EventStorming starts from discovery or an existing thesis and separates confirmed business facts from a current falsifiable structural model. When—and only when—that model leaves a real Design Delta, Tactical Design first establishes a domain-object UML, responsibilities, state authority, semantic flow, and necessity proof, then derives the fewest critical sequences. Its `draft` is an exploration candidate: Codify may test it through reversible implementation, Tactical Design reconciles the evidence, and only the resulting `ready` design can enter final verification and Guard. House Style supplies conditional realization rules rather than choosing scenario-specific lifecycle or business sequencing.
