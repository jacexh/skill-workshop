...
Tactical Design is a relentless one-question-at-a-time interview. It works one Aggregate Root at a time, derives essential business pressures from confirmed rules, probes behavior ownership through subject-action-object-result statements, and compares credible object compositions by pressure coverage and accidental design burden. It records each retained object's definition, state, behavior, and actual Domain Events, then writes that Root's current slice immediately after confirmation. An accepted behavior description names its owning object as subject and becomes that object's intention-revealing method by default; receiver-shaped free functions require a concrete ownership reason.
