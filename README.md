...
Standalone hookless DDD/backend architecture expert skills for code agents. EventStorming is the single modeling workflow: it keeps discovery on a temporary EventStorming Board, runs the ten EventStorming steps through one frontier question at a time, adversarially reviews the complete integrated model, then writes one `draft` meeting record as the approval surface. Confirmation changes those minutes to `ready` and synchronizes affected canonical Models and project documents. Each Model preserves Role-to-Command permissions, assigns normalized Commands to Aggregate Capabilities, and retains only business-required fact-to-Command causality. When—and only when—the Model leaves a real Design Delta, Tactical Design uses typed critical sequences as the Model-to-design projection and confirms ownership plus stable claims; only durable BC-specific decisions are projected into optional sibling `architecture.md` files, while every Tactical Design claim is explicitly classified as durable or iteration-only. Ordinary reversible work goes directly to Codify and its verified implementation checkpoint. Codify carries a temporary many-to-many Model-to-code projection for the implementation; Guard independently reconstructs it over the final stable snapshot and jointly closes clear iteration records when the producer checkpoint is complete.

Invoke EventStorming with a user story, business scenario, Spec, PRD, or existing Model. It proceeds from analytical, past-tense Workshop Events through timeline, Commands, actors/external systems, rules, Hotspots, Aggregates/core objects, Bounded Contexts, and collaboration; only selected local facts become Domain Event semantics in canonical Models. The facilitator investigates available evidence, asks the user one material fact or decision at a time, recommends design choices with their strongest credible alternative, and allows later evidence to reopen working conclusions. Package names, services, tables, runtime components, and call direction are evidence rather than context boundaries. Repeated mechanisms are evaluated with DRY-as-knowledge, cohesion, information hiding, coupling, and YAGNI before choosing shared domain ownership, a shared technical Module, or distinct local semantics with translations. The Context Map records the acyclic semantic `U -> D` dependency graph and its named contracts; complete cross-context scenario interactions remain in the iteration minutes. Canonical Models preserve current context-owned conclusions, increment `model_revision`, and link their latest minutes through `last_changed_by`.

In Codify, house-style guidance keeps Domain/Application/Interface/Infrastructure/Runtime responsibilities explicit while current BC Architecture and accepted Tactical Design constrain material collaboration. The temporary projection maps Model meaning to responsibilities and `path#symbol` evidence without forcing same-named classes or exact method counts. Project evidence determines reversible Repository/CQRS details, persistence mapping, adapters, runtime wiring, migrations, and verification. `Interface` is the language-neutral responsibility; Go names its physical package `transport`. Guard independently checks those accepted authorities and the projection in House Style; it is not a comprehensive code review or verification campaign.
