...
In Codify, house-style guidance keeps Domain/Application/Interface/Infrastructure/Runtime responsibilities explicit while current BC Architecture and accepted Tactical Design constrain material collaboration. Project evidence determines reversible Repository/CQRS details, persistence mapping, adapters, runtime wiring, migrations, and verification. `Interface` is the language-neutral responsibility; Go names its physical package `transport`. Guard independently checks those accepted authorities in House Style; it is not a comprehensive code review or verification campaign.
