...
Standalone hookless DDD/backend skills for code agents. EventStorming retains the complete ten-step discussion method and produces a compact current strategic model: Bounded Contexts, Aggregate Roots, essential language, business rules, and semantic dependencies. Each Business Rule is an independently challengeable business claim that can drive downstream design without assigning tactical ownership. When an occurrence requires or enables a later intent, EventStorming preserves whether the first occurrence is already a successful business fact. Workshop Events and discussion history remain conversational rather than becoming permanent artifacts.

Tactical Design is a relentless one-question-at-a-time interview. It works one Aggregate Root at a time, derives essential business pressures from confirmed rules, probes behavior ownership through `<Subject> <domain verb> <Object>.` statements, applies the Capability Probe to external-authority needs, classifies every material causal continuation before choosing a collaboration mechanism, and compares credible object compositions by pressure coverage and accidental design burden. The Capability Probe selects a Supplied Fact when ownership, timing, authority, and guarantee remain intact, or a Required Capability directly invoked by the Behavior; each capability records its Behavior, business decision point, Domain question or action, and guarantee. The continuation classification distinguishes one success guarantee, an independent local reaction, a cross-context published fact, and no selected event; the need for durable occurrence evidence is tested independently, and Aggregate or Bounded Context membership alone does not decide. When a local reaction is selected, its Domain Event becomes the causal handoff instead of an observation beside the original direct path. Every candidate Entity proposal explains what the object is and how it operates, then introduces its Behaviors in Domain language at that first proposal rather than renaming technical placeholders during artifact writing. Simple operation stays concise, while an essential operating characteristic may become part of Definition. It records each coherent Entity's definition, Facts, Lifecycle State, behavior, Required Capabilities where present, and actual Domain Events in `domain-objects.md` as the Root develops, then confirms the integrated Root and updates accepted changes across affected current DDD artifacts and project decisions. An accepted behavior names any Lifecycle State transition and becomes its subject's method by default; a selected Domain Event points only to the behavior that records it; receiver-shaped free functions require a concrete ownership reason.

Codify treats the strategic model and domain-object slices as read-only semantic constraints, not a complete software design. It implements the requested behavior and fills the remaining software structure through project and active-language House Style, loading guidance only for code actually touched. Domain/Application/Interface/Infrastructure/Runtime responsibilities remain explicit; Go names the physical Interface package `transport`. Guard independently reviews whether the implementation preserves the accepted model and whether changed non-Domain abstractions earn their cost by reducing overall complexity under House Style; it does not modify code or DDD artifacts.
