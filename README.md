...
Standalone hookless DDD/backend skills for code agents. EventStorming retains the complete ten-step discussion method and produces a compact current strategic model: Bounded Contexts, Aggregate Roots, essential language, business rules, and semantic dependencies. Workshop Events and discussion history remain conversational rather than becoming permanent artifacts.

Tactical Design is a relentless one-question-at-a-time interview. It works depth-first through one Aggregate Root and all of its Entities, records each object's definition, state, behavior, and actual Domain Events, and writes that Root's current slice immediately after confirmation. A behavior description names its owning object as subject and becomes that object's intention-revealing method by default; receiver-shaped free functions require a concrete ownership reason.
