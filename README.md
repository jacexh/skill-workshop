...
Tactical Design is a relentless one-question-at-a-time interview. It works one Aggregate Root at a time, derives essential business pressures from confirmed rules, probes behavior ownership through `<Subject> <domain verb> <Object>.` statements, and compares credible object compositions by pressure coverage and accidental design burden. It records each retained object's definition, Facts, Lifecycle State, behavior, and actual Domain Events, then writes that Root's current slice immediately after confirmation. Facts are business-significant object knowledge rather than a field inventory; Lifecycle State records explicit state-machine states or that none exists. An accepted behavior uses an intention-revealing Domain verb, names any Lifecycle State transition, and becomes its subject's method by default; a selected Domain Event points only to the behavior that records it; receiver-shaped free functions require a concrete ownership reason.
