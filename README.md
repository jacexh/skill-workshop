...
Standalone hookless DDD/backend architecture expert skills for code agents. EventStorming is the single modeling workflow: it keeps discovery on a temporary EventStorming Board, runs the ten EventStorming steps through one frontier question at a time, adversarially reviews the complete integrated model, then writes one `draft` meeting record as the approval surface. Confirmation changes those minutes to `ready` and synchronizes the affected canonical Models and project documents. Codify realizes the ready iteration and runs producer conformance before a verified implementation checkpoint. Guard uses one fresh read-only reviewer to judge finite semantic-structure units over the final stable snapshot and, when its matching producer checkpoint is complete, marks a structurally clear iteration `implemented` at an iteration-closure or merge/release certification boundary.
...
In Codify, house-style guidance keeps Domain/Application/Interface/Infrastructure/Runtime responsibilities explicit while project evidence determines Repository/CQRS shape, ports, persistence, adapters, runtime wiring, migrations, and verification. `Interface` is the language-neutral responsibility; Go names its physical package `transport`. Guard independently checks whether the accepted Model is expressed in that House Style; it is not a comprehensive code review or verification campaign.
