...
Standalone hookless DDD/backend architecture expert skills for code agents. EventStorming is the single Strategic Modeling workflow: it keeps discovery on a temporary EventStorming Board, runs the ten EventStorming steps through one frontier question at a time, adversarially reviews the complete integrated model, and waits for explicit model confirmation before any project write. Codify realizes separately accepted Tactical Design, and Guard performs parallel Design Realization/House-Style Conformance review. Every material Codify change completes through a clear independent Guard in the same task.

Invoke EventStorming with a user story, business scenario, Spec, PRD, or existing Model. It proceeds from past-tense events through timeline, Commands, actors/external systems, rules, Hotspots, Aggregates/core objects, Bounded Contexts, and collaboration. The facilitator investigates available evidence, asks the user one material fact or decision at a time, recommends design choices with their strongest credible alternative, and allows later evidence to reopen working conclusions. Package names, services, tables, runtime components, and call direction are evidence rather than context boundaries. Repeated mechanisms are evaluated with DRY-as-knowledge, cohesion, information hiding, coupling, and YAGNI before choosing shared domain ownership, a shared technical Module, or distinct local semantics with translations. The Context Map separates its acyclic semantic `U -> D` dependency graph from runtime/business initiator-to-receiver interactions. Only explicit confirmation of the complete integrated model authorizes one synchronized documentation closure across relevant DDD, Spec, PRD, ADR, and Glossary artifacts. Written Models preserve the confirmed diagram and use `model_status: model_ready`. EventStorming then stops—Tactical Design is not generated implicitly.

When separately accepted design enters Codify, house-style guidance keeps Domain/Application/Transport/Infrastructure responsibilities explicit and Guard independently checks both realization and conformance.
