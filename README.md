...
Codify treats the strategic model and domain-object slices as read-only authority. Implementation evidence that contradicts them stops coding and returns to the appropriate modeling decision. House Style supplies conditional realization rules without inventing transaction, concurrency, recovery, or runtime complexity. Domain/Application/Interface/Infrastructure/Runtime responsibilities remain explicit; Go names the physical Interface package `transport`. Guard independently reviews the completed implementation without modifying code or DDD artifacts.
