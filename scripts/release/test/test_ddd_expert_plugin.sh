...
check_review_evidence_gate() {
  ...
  grep -q "Counterfactual defect hunt: Checked row | Falsifier question | Evidence inspected | Decision" "$review_skill" || fail "$label review output template should require a falsification ledger"
  ...
  grep -q "Checked row | Risk family | Required proof artifact | Evidence | Decision" "$review_skill" || fail "$label review output template should require proof artifact ledger columns"
  grep -q "Terminal/execution fact table: Flow | Execution fact | Authorization source | Amount/result scope | Idempotency/replay rule | Failure recovery | Aggregate closure condition | Decision" "$review_skill" || fail "$label review output should require terminal/execution fact proof"
  grep -q "CQRS semantic split table: Port/interface | Caller semantics | Returned model family | Write-side overlap | Adapter overlap | Decision" "$review_skill" || fail "$label review output should require CQRS semantic proof"
  ...
  grep -q "Output completion gate: Exact lifecycle section | Present | Non-empty | If missing decision" "$review_skill" || fail "$label review output template should require completion gate columns"
  ...
  grep -q "Negative decision inventory: Row | Initial decision | Evidence inspected | Row-local promotion proof | Final decision | Finding id" "$review_skill" || fail "$label review output should include negative decision inventory columns"
  ...
  grep -q "Overclaim scrub: Checked row | Strongest evidence source | Forbidden promotion source | Downgrade decision" "$review_skill" || fail "$label review output should include overclaim scrub table"
  grep -q "Checked row admission control: Row | Proof tuple complete | Invalid category/prose/forbidden proof | Admission decision" "$review_skill" || fail "$label review output should include checked-row admission-control table"
