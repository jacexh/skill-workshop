...
assert_before() {
  ...
  first_match="$(rg -n -m 1 -F -- "$first" "$file" || true)"
  second_match="$(rg -n -m 1 -F -- "$second" "$file" || true)"
  [ -n "$first_match" ] || fail "$label missing first contract: $first"
  [ -n "$second_match" ] || fail "$label missing second contract: $second"
  [ "${first_match%%:*}" -lt "${second_match%%:*}" ] || fail "$label"
}
...
assert_contains "$CLAUDE_ROOT/skills/codify/SKILL.md" 'Run Guard once over that stable snapshot rather than after each edit' "codify should batch independent review at a certification boundary"
...
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'exactly one fresh minimal-history Guard coordinator' "guard should isolate coordination from implementation history"
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'only the scoped request, stable source snapshot handoff, and artifact paths' "guard coordinator should receive a bounded handoff"
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'do not launch or hand Guard to another coordinator' "guard should not replay a fresh coordinator through another handoff"
...
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" '**Record mechanical evidence once**' "guard coordinator should own shared mechanical verification"
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'Run available build, test, lint, diff, model, and other mechanical checks once under the coordinator' "guard should not duplicate broad mechanical checks across workers"
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'Workers consume recorded mechanical evidence' "guard workers should reuse coordinator-owned checks"
assert_before "$CLAUDE_ROOT/skills/guard/SKILL.md" '**Record mechanical evidence once**' '**Freeze one Review Envelope**' "guard should record shared mechanical evidence before freezing the envelope"
assert_before "$CLAUDE_ROOT/skills/guard/SKILL.md" '**Freeze one Review Envelope**' '**Launch both axes**' "guard should freeze the complete envelope before worker dispatch"
...
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'fresh minimal-history contexts with no inherited conversation' "guard workers should not replay coordinator history"
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'Pass only the compact Review Envelope' "guard workers should receive bounded review input"
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'completion notification or one long wait sized to the expected axis runtime' "guard should wait in an event-driven or long-lived turn"
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'repeated short status, list, or message polling' "guard should reject high-replay polling loops"
...
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'coordinator executes each justified probe at most once' "guard should centralize targeted additional verification"
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'request the smallest targeted additional probe in `depth_scope`' "guard workers should justify extra probes through a finding"
...
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'A coverage row is a `proof family`' "guard should group inventory IDs that share one proof"
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'group every inventory ID sharing one evidence and reason' "guard should avoid repeating proof for additive inventory views"
...
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'retry it once in a fresh minimal-history worker with the same compact envelope' "guard retries should not inherit failed worker history"
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'A repaired source snapshot starts a new Guard execution with a new envelope' "guard should not reuse stale coverage after repairs"
...
assert_contains "$ROOT/README.md" 'runs producer conformance before a verified implementation checkpoint' "root README should expose the Codify checkpoint"
assert_contains "$ROOT/README.md" 'iteration-closure or merge/release certification boundary' "root README should expose the Guard certification boundary"
...
assert_contains "$ROOT/docs/adr/0001-ddd-expert-reference-architecture.md" 'fresh minimal-history contexts' "DDD ADR should bound Guard history replay"
assert_contains "$ROOT/docs/adr/0001-ddd-expert-reference-architecture.md" 'records shared mechanical verification once before dispatch' "DDD ADR should centralize Guard mechanical verification"
assert_contains "$ROOT/docs/adr/0001-ddd-expert-reference-architecture.md" 'proof families' "DDD ADR should allow shared proof without weakening exact coverage"
