...
check_review_evidence_gate() {
  ...
  grep -q "do not omit this row because recovery or durable-fact command admission already has a finding" "$review_skill" || fail "$label review should not let payment findings hide state vocabulary"
