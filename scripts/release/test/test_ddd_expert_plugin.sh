...
check_review_evidence_gate() {
  ...
  local label="$2"
  grep -q "Do not combine with project-memory lookup unless the user explicitly asks for memory" "$review_skill" || fail "$label review description should block default memory lookup"
  ...
  grep -q "Risk router is a bounded routing index, not a full-file preload" "$review_skill" || fail "$label review should not preload the whole risk router"
  grep -q "Do not invoke superpowers-memory:query or read project memory files during review unless the user explicitly asks for memory" "$review_skill" || fail "$label review should not load project memory by default"
  grep -q "Already-injected memory context is a routing hint, not authority or a reason to open KB files" "$review_skill" || fail "$label review should treat injected memory as hints only"
  grep -q "If a reference, memory file, or broad source would require unbounded reading, emit an evidence gap row and continue" "$review_skill" || fail "$label review should continue instead of reading until final output disappears"
  ! grep -q "First read \\[../../references/ddd-risk-router.md\\].*once" "$review_skill" || fail "$label review should not require full risk-router preload"
  ...
  grep -q "Axis ledgers are evidence-indexed, not exhaustive source traversal" "$review_skill" || fail "$label review should not interpret axis completion as exhaustive source traversal"
  grep -q "Use rg inventories before opening files" "$review_skill" || fail "$label review should prefer inventories before file reads"
  grep -q "Do not read long implementation, test, generated, migration, or SQL files end to end before final output" "$review_skill" || fail "$label review should forbid broad long-file reads before final output"
  grep -q "One row-local snippet is enough to classify a row; otherwise emit evidence gap and continue" "$review_skill" || fail "$label review should convert missing proof to evidence gaps instead of more reading"
  grep -q "Final-output checkpoint: after the first model/spec/diff inventory batch, emit the DDD review from the current axis ledger" "$review_skill" || fail "$label review should emit final after the first evidence inventory"
  grep -q "Do not start a second broad evidence batch before final output" "$review_skill" || fail "$label review should not run repeated broad evidence batches"
  grep -q "Never end a review turn after tool output with reasoning only" "$review_skill" || fail "$label review should not end without final output"
