...
jq -e '.interface.defaultPrompt | all(.[]; contains("one frontier decision at a time") and contains("strongest credible alternative") and contains("model_ready") and contains("$ddd-expert:codify"))' "$CODEX_ROOT/.codex-plugin/plugin.json" >/dev/null || fail "Codex ddd-expert default prompt should preserve HITP modeling and direct implementation readiness"
...
for template in README artifact-layout context-map model; do
...
assert_contains "$event_storming_skill" '`validate-proposed-model` after adversarial review, `write-model-draft` to materialize the approval candidate, and `apply-confirmed-model` only after the user confirms that exact draft' "EventStorming should phase draft materialization, approval, and synchronization explicitly"
...
assert_contains "$event_storming_skill" 'replace each affected canonical `model.md` with the complete candidate, increment `model_revision`, and set `model_status: draft`' "EventStorming should write canonical draft Models before approval"
assert_contains "$event_storming_skill" 'In the console, summarize the scope, draft paths and revisions' "EventStorming should summarize file-backed approval candidates"
assert_contains "$event_storming_skill" 'promote the exact displayed draft to `model_ready` without another revision increment' "EventStorming should promote the approved draft revision in place"
...
assert_contains "$event_storming_skill" 'A `model_ready` result is ready for Codify' "EventStorming completion should hand the confirmed Model directly to Codify"
assert_contains "$CLAUDE_ROOT/skills/codify/SKILL.md" 'A canonical `model_ready` Model is sufficient implementation authority' "Codify should accept a confirmed Model without another readiness artifact"
...
assert_contains "$event_storming_skill" 'scenario interactions in the affected `model.md` diagrams' "EventStorming should keep relevant runtime interaction evidence in the scenario model"
...
assert_contains "$event_storming_skill" 'no additional readiness artifact or workflow stage is required' "EventStorming should not insert a separate design phase before Codify"
assert_not_contains "$event_storming_skill" 'codify_ready' "EventStorming should expose only model_ready as implementation readiness"
...
assert_contains "$CLAUDE_ROOT/templates/model.md" 'model_status: draft' "model template should materialize the approval candidate as a draft"
assert_contains "$CLAUDE_ROOT/templates/model.md" 'Use model_status: draft before confirmation and promote the exact approved revision to model_ready' "model template should support file-backed draft approval"
...
assert_contains "$CLAUDE_ROOT/templates/artifact-layout.md" 'A canonical `model_ready` Model enters Codify directly' "artifact layout should define the direct EventStorming-to-Codify handoff"
...
assert_contains "$CLAUDE_ROOT/templates/README.md" 'A `model_ready` Model is ready for Codify' "artifact README should expose the direct implementation handoff"
...
assert_contains "$CLAUDE_ROOT/skills/codify/SKILL.md" 'a canonical `model_ready` Model enters Codify directly' "codify should enter from model_ready without another gate"
assert_contains "$CLAUDE_ROOT/skills/codify/SKILL.md" 'Route a `draft_model`' "codify should reject unconfirmed canonical draft Models"
...
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'A `draft_model`, `legacy_model`, or missing Model is not ready implementation authority' "guard should require only confirmed Model authority"
...
assert_contains "$claude_maintainer" 'event-storming` may use `inspect`, `validate-proposed-model`, `write-model-draft`, and `apply-confirmed-model' "artifact maintainer should authorize EventStorming operations"
...
assert_contains "$claude_maintainer" '`draft_model`: a structurally valid unconfirmed Model' "artifact inspection should expose canonical draft review state"
assert_contains "$claude_maintainer" 'promotion of the exact approved draft keeps the same revision' "artifact maintainer should promote approved drafts without revision churn"
assert_contains "$claude_maintainer" '`legacy_model`' "artifact inspection should expose pre-model_ready Models"
assert_contains "$claude_maintainer" '`model_ready`: a structurally valid confirmed Model' "artifact inspection should expose the canonical direct-Codify state"
