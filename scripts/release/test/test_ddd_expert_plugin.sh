...
check_review_evidence_gate() {
  ...
  grep -q "\\*\\*Report\\*\\*: turn the synthesized verdicts into findings, evidence gaps / returns, non-smell positive notes, verification, and residual risk\\. Output: final review judgment" "$review_skill" || fail "$label review workflow should output final review judgment"
  ...
  grep -q "Every explained smell-family verdict lands in Findings or Evidence gaps / returns" "$review_skill" || fail "$label review should not drop explained smell-family verdicts"
  ...
  grep -q "Repository/API smells need method inventory" "$review_skill" || fail "$label review output should require repository method inventory before no-finding"
  ...
  grep -q "CQRS smells need write/read method inventory" "$review_skill" || fail "$label review output should require CQRS method inventory before no-finding"
