...
check_review_evidence_gate() {
  ...
  grep -q "durable-fact command admission, terminal/execution split, repository/API candidate owner, collaboration mechanism, parent state vocabulary, and CQRS inventory" "$review_skill" || fail "$label review workflow should seed required family rows"
  ...
  grep -q "\\*\\*Report\\*\\*: turn the synthesized verdicts into findings, evidence gaps / returns, non-smell positive notes, verification, and residual risk\\. Output: final review judgment that accounts for every required family row" "$review_skill" || fail "$label review workflow should output final review judgment"
  ...
  grep -q "cite smell families in findings, evidence gaps, returns, or no-finding notes with observed correct shape" "$review_skill" || fail "$label review should cite smell families in decisions"
  grep -q "Every required family row and every explained smell-family verdict lands in Findings, Evidence gaps / returns, or a no-finding note with observed correct shape" "$review_skill" || fail "$label review should not drop required family rows"
  ...
  grep -q "Do not collapse production wiring, collaboration mechanism, candidate-owner, state vocabulary, or CQRS method-inventory decisions into a broader claim" "$review_skill" || fail "$label review should not hide smell-family decisions behind broader findings"
  ...
  grep -q "No-finding notes are only for non-smell surfaces or observed correct shape" "$review_skill" || fail "$label review output should require observed correct shape for no-finding notes"
  ...
  grep -q "If the positive proof is only package names, object splitting, accepted design, DTO presence, QueryRepository presence, or passing tests, report an evidence gap" "$review_skill" || fail "$label review output should downgrade weak no-finding proof to evidence gap"
