...
check_review_evidence_gate() {
  ...
  grep -q "Evidence-gap is for missing proof; if inspected evidence shows only a forbidden shape or no required mechanism" "$review_skill" || fail "$label review should not downgrade negative evidence to pressure"
  grep -q "output violation, return-to-domain-modeling, or return-to-design instead of pressure" "$review_skill" || fail "$label review workflow should upgrade actual negative evidence"
  ...
  grep -q "Collaboration mechanism: command transaction, synchronous save, or repository call is coordination evidence, not the mechanism" "$review_skill" || fail "$label review should not treat command transactions as collaboration mechanisms"
  grep -q "repeated cross-lifecycle reactions require a named Domain Event, process manager, reconciler, task processor, Integration Message, explicit operator command, or return/evidence gap" "$review_skill" || fail "$label review should require named collaboration mechanisms"
  grep -q "adjacent findings do not clear this row" "$review_skill" || fail "$label review should not let adjacent findings clear collaboration"
  ...
  grep -q "Negative evidence upgrade: actual evidence of command-transaction-only coordination, write-side read helpers, parent states named after child outcomes, or missing runtime/collaboration mechanism is a final verdict input" "$review_skill" || fail "$label review should treat actual negative evidence as final verdict input"
  grep -q "not mere pressure or inventory" "$review_skill" || fail "$label review should not leave negative evidence as pressure or inventory"
  grep -q "Required family rows: lifecycle/repository/event/CQRS scope keeps durable-fact command admission" "$review_skill" || fail "$label review should require separate family rows for lifecycle scopes"
  ...
  grep -q "write-side read helpers are negative evidence until caller semantics and returned model family prove command-side aggregate fact lookup" "$review_skill" || fail "$label review should treat write-side read helpers as CQRS negative evidence"
  grep -q "Parent-state vocabulary: parent states named with child execution or attempt outcomes require a parent lifecycle fact" "$review_skill" || fail "$label review should require generic parent-state vocabulary rows"
  grep -q "command-admission, recovery, or terminal-event findings do not clear this row" "$review_skill" || fail "$label review should not let adjacent findings hide state vocabulary"
  ! grep -q "Payment exists beside TaskAgreement" "$review_skill" || fail "$label review should not hard-code sanhe payment vocabulary"
  ! grep -q "payment_pending" "$review_skill" || fail "$label review should not hard-code sanhe payment states"
  ! grep -q "delivery/refund/dispute/settlement" "$review_skill" || fail "$label review should not hard-code sanhe collaboration flows"
  ...
  grep -q "Pressure, inventory, and command-side lookup are intermediate review evidence" "$review_skill" || fail "$label review should treat pressure inventory and command-side lookup as intermediate evidence"
  grep -q "upgrade them to a finding or return instead of leaving them as a weak evidence gap" "$review_skill" || fail "$label review should upgrade concrete negative evidence in final output"
