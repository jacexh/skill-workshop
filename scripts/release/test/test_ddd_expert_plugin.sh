...
check_explore_skill() {
  ...
  assert_references_last "$explore_skill" "$label explore"
...
check_shape_skill() {
  ...
  assert_references_last "$shape_skill" "$label shape"
  ...
  grep -q "^1\\. Read project inputs:" "$shape_skill" || fail "$label shape workflow should start from project evidence"
  ...
  grep -q "Establish phase fit from project evidence before loading references" "$shape_skill" || fail "$label shape should establish phase fit before references"
  grep -q "^## References$" "$shape_skill" || fail "$label shape should route references in its final section"
  grep -q "After phase fit succeeds, load only the references needed by the touched design surface" "$shape_skill" || fail "$label shape should route references after phase fit"
  ...
  grep -q "^2\\. Check phase fit:" "$shape_skill" || fail "$label shape workflow should list phase fit as a step"
  grep -q "Do not run discovery or batch questions from \`shape\`" "$shape_skill" || fail "$label shape should return model discovery without batching questions"
  grep -q "single highest-leverage missing model fact" "$shape_skill" || fail "$label shape should identify one upstream model gap"
  ...
  grep -q "^3\\. Shape decisions:" "$shape_skill" || fail "$label shape workflow should list shaping as a step"
  grep -q "accepted model facts first" "$shape_skill" || fail "$label shape should start from accepted model facts"
  grep -q "^4\\. Gate-review output:" "$shape_skill" || fail "$label shape workflow should list gate review as a step"
...
check_codify_skill() {
  ...
  assert_references_last "$codify_skill" "$label codify"
  ...
  grep -q "^4\\. \\*\\*Load touched references\\*\\* — after surface preflight" "$codify_skill" || fail "$label codify should load references after surface preflight"
...
check_review_evidence_gate() {
  ...
  assert_references_last "$review_skill" "$label guard"
  ...
  grep -q "^## Scope$" "$review_skill" || fail "$label guard should define its review scope before workflow"
  grep -q "^## References$" "$review_skill" || fail "$label guard should route references in its final section"
  ...
  grep -q 'report an evidence gap and route to `explore` instead of inventing a model' "$review_skill" || fail "$label guard should route missing model evidence to explore"
  grep -q 'report an evidence gap and route to `shape`' "$review_skill" || fail "$label guard should route missing tactical placement to shape"
  ...
  ! grep -q "First read \\[../../references/ddd-core.md\\]" "$review_skill" || fail "$label review should not load core before breadth scan"
  grep -q "Guard owns the workflow and breadth-scan projection" "$review_skill" || fail "$label review should own the breadth-scan projection"
  grep -q "ddd-core.md.*owns the normative depth rules" "$review_skill" || fail "$label core should own normative depth rules"
  grep -q 'Do not load `ddd-core.md` wholesale before the breadth scan' "$review_skill" || fail "$label review should delay core loading until depth"
  grep -q "After same-shape smells are merged, load only the references needed" "$review_skill" || fail "$label guard should route deeper references after smell merge"
  ...
  grep -q "becomes a smell candidate, never a finding or evidence gap by itself" "$review_skill" || fail "$label breadth baseline should produce candidates only"
  grep -q "Use references only to prove or clear triggered smells and depth obligations, not to enumerate findings" "$review_skill" || fail "$label review should not enumerate findings from references"
  ...
  grep -q "\\*\\*Breadth scan\\*\\*: compare touched code shape against the layer baseline and seed every applicable specialized house-style depth obligation\\. Output: Smell List rows" "$review_skill" || fail "$label review workflow should output smell rows and specialized depth obligations from breadth"
  ...
  grep -q "\\*\\*Merge and group\\*\\*: merge smell rows by owner, lifecycle, boundary, state vocabulary, collaboration mechanism" "$review_skill" || fail "$label review workflow should merge smells and group depth obligations"
  ...
  grep -q "\\*\\*Deep-check smells and obligations\\*\\*: load only the relevant" "$review_skill" || fail "$label review workflow should load references per smell family and depth obligation"
  grep -q "For a smell, assume it is wrong until" "$review_skill" || fail "$label review workflow should deep-check smells with guilty-presumption shape"
  grep -q "output violation, return-to-explore, return-to-shape, evidence-gap, or adjacent-smell" "$review_skill" || fail "$label review workflow should output constrained smell verdicts"
  grep -q "clear it internally when all applicable rules conform or are not applicable" "$review_skill" || fail "$label review workflow should allow clean depth obligations"
  grep -q "promote only a concrete rule miss to a Smell List row" "$review_skill" || fail "$label review workflow should promote only concrete specialized misses"
  ...
  grep -q "Output: updated Smell List and Depth List" "$review_skill" || fail "$label review workflow should update both breadth lists"
  grep -q "If this adds a row or obligation, return to Step 2" "$review_skill" || fail "$label review workflow should deep-check rows and obligations found through adjacent evidence"
  ...
  grep -q "### Specialized House-Style Baseline" "$review_skill" || fail "$label review should seed specialized house-style candidates during breadth"
  grep -q "Seed one depth obligation for every touched specialized surface during breadth" "$review_skill" || fail "$label review should cover every touched specialized surface"
  grep -q "Database:.*seeds a database-profile obligation" "$review_skill" || fail "$label review should route touched database evidence into depth"
  grep -q "Agent contract:.*seeds an agent-contract obligation" "$review_skill" || fail "$label review should route touched agent-contract evidence into depth"
  grep -q "touched-surface classification alone never proves conformance or violation" "$review_skill" || fail "$label review should keep specialized surface rows non-verdict candidates"
  grep -q "Only a concrete suspected rule miss enters the Smell List" "$review_skill" || fail "$label review should keep clean specialized obligations out of the smell list"
  grep -q "### Layer Baseline" "$review_skill" || fail "$label review should define layer baseline"
  grep -q "high-recall scan projection of \`ddd-core.md\`, not a conformance verdict" "$review_skill" || fail "$label baseline should be framed as a scan projection"
  grep -q "which required shapes may be missing, and which forbidden shapes appear" "$review_skill" || fail "$label review should derive smell candidates from missing-required and present-forbidden shapes"
  grep -q "A sentinel match is not a verdict" "$review_skill" || fail "$label sentinels should require family deep checks"
