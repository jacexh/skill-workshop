...
assert_contains "$event_storming_skill" 'connected `actor/external -> Command -> Aggregate Capability or explicit coordination -> policy/rule -> Workshop Event` thread' "EventStorming should connect normalized intent through Root-owned behavior and rules in every causal thread"
...
assert_contains "$CLAUDE_ROOT/references/ddd-modeling.md" 'Normalize Commands by the intent chosen by the actor or issuing policy' "DDD modeling guidance should normalize repeated scenario commands before capability projection"
...
assert_contains "$CLAUDE_ROOT/templates/event-storming.md" '## Command-to-Capability Projection' "EventStorming minutes should expose compact capability traceability"
...
assert_contains "$CLAUDE_ROOT/templates/event-storming.md" 'Aggregate Capability: <Root-owned operation>' "EventStorming diagrams should visualize the Command-to-Capability relationship"
assert_contains "$CLAUDE_ROOT/templates/event-storming.md" 'actor --> command --> capability --> policy --> event' "EventStorming diagrams should place intent before Root-owned behavior and decision rules"
assert_contains "$CLAUDE_ROOT/templates/event-storming.md" '| Bounded Context | Normalized Command | Aggregate Root or coordination owner | Aggregate Capability or explicit coordination |' "EventStorming minutes should keep Command-to-Capability mapping checkable"
...
assert_contains "$CLAUDE_ROOT/templates/event-storming.md" 'Do not repeat required facts, outcomes, rejection branches, or other capability contract fields here' "EventStorming minutes should leave the full capability contract to the Model"
...
assert_contains "$event_storming_skill" 'every in-scope Command is accounted for, every new or changed capability has a source Command' "EventStorming should validate Command-to-Capability coverage in both directions"
...
assert_contains "$CLAUDE_ROOT/skills/codify/SKILL.md" 'trace each normalized business Command through its confirmed Aggregate Capability or explicit coordination' "codify should realize the accepted Command-to-Capability mapping"
assert_contains "$CLAUDE_ROOT/skills/codify/SKILL.md" 'Do not derive a method inventory from scenario branches or treat each capability row as an exact method count' "codify should not turn Model rows into mechanical method cardinality"
...
assert_contains "$claude_maintainer" 'compact Command-to-Capability mapping and Required Reaction projection' "proposal validation should keep iteration capability traceability compact"
...
assert_contains "$claude_maintainer" 'every source Command named by an in-scope capability to occur in a connected scenario thread' "proposal validation should trace capabilities to real EventStorming Commands"
assert_contains "$claude_maintainer" 'every in-scope state-changing Command to reach a capability or explicit coordination' "proposal validation should close Command ownership gaps"
assert_contains "$claude_maintainer" 'supported by the connected scenario threads rather than introduced only in the Model projection' "proposal validation should reject unreviewed capability meaning"
