...
assert_contains "$CLAUDE_ROOT/templates/artifact-layout.md" 'exhaustive Architecture dispositions' "artifact layout should make Tactical Design claim disposition part of the record"
assert_contains "$CLAUDE_ROOT/templates/artifact-layout.md" 'Perform no Architecture write only when every claim is `iteration-only`' "artifact layout should reject silent projection omission"
...
assert_contains "$tactical_design_skill" 'Group all technical objects owned by the same Bounded Context in one Mermaid `box ... end`' "Tactical Design should group technical objects by Bounded Context"
assert_contains "$tactical_design_skill" 'Label every invocation arrow into or between technical participants `Public Method: <receiver method>(<semantic inputs>)`' "Tactical Design should name receiver Public Methods on invocation arrows"
assert_contains "$tactical_design_skill" 'label every reply arrow `Returns: <semantic result>`' "Tactical Design should distinguish returns from method invocations"
assert_contains "$tactical_design_skill" 'adjacent traceability notes and participant labels' "Tactical Design should keep Model meaning adjacent to method calls"
...
assert_contains "$tactical_design_skill" 'Account for every Tactical Design Claim exactly once' "Tactical Design should make Architecture disposition exhaustive"
assert_contains "$tactical_design_skill" 'Perform no Architecture write only when every claim is explicitly `iteration-only`' "Tactical Design should require an explicit reason before skipping Architecture synchronization"
...
assert_contains "$tactical_design_template" 'status: draft' "Tactical Design records should begin as drafts"
assert_contains "$tactical_design_template" '## Critical Collaboration Sequences' "Tactical Design template should require sequence diagrams"
assert_contains "$tactical_design_template" 'sequenceDiagram' "Tactical Design template should use Mermaid sequence diagrams"
assert_contains "$tactical_design_template" 'only persisted Model-to-design projection' "Tactical Design template should keep projection in sequences only"
assert_contains "$tactical_design_template" 'actor Role as Role: <Business Role>' "Tactical Design template should distinguish a business Role from technical participants"
assert_contains "$tactical_design_template" 'box <Bounded Context>' "Tactical Design template should frame one Bounded Context's technical objects"
tactical_design_bc_box="$(
  awk '
    /^[[:space:]]*box <Bounded Context>$/ { in_box = 1 }
    in_box { print }
    in_box && /^[[:space:]]*end$/ { exit }
  ' "$tactical_design_template"
)"
for participant in 'participant Interface' 'participant Application' 'participant Repository' 'participant Root'; do
  [[ "$tactical_design_bc_box" == *"$participant"* ]] || fail "Tactical Design template should keep $participant inside the Bounded Context box"
done
[[ "$tactical_design_bc_box" != *'actor Role'* ]] || fail "Tactical Design template should keep the business Role outside technical-object boxes"
assert_contains "$tactical_design_template" 'Role->>Interface: Public Method: <InboundMethod>(<Request>)' "Tactical Design template should name the inbound Public Method"
assert_contains "$tactical_design_template" 'Interface->>Application: Public Method: <UseCaseMethod>(<Command>)' "Tactical Design template should name the Application Public Method"
assert_contains "$tactical_design_template" 'Application->>Repository: Public Method: Get(<Aggregate identity>)' "Tactical Design template should name the Repository read method"
assert_contains "$tactical_design_template" 'Application->>Root: Public Method: <CapabilityMethod>(<Domain arguments>)' "Tactical Design template should name the Root Public Method"
assert_contains "$tactical_design_template" 'Application->>Repository: Public Method: Save(<Aggregate Root>)' "Tactical Design template should name the Repository write method"
assert_contains "$tactical_design_template" 'Repository-->>Application: Returns: <Aggregate Root>' "Tactical Design template should label reply arrows as returns"
assert_contains "$tactical_design_template" 'Note over Role,Application: Command: <Business intent>' "Tactical Design template should preserve Command traceability beside method calls"
assert_contains "$tactical_design_template" 'Note over Application,Root: Capability: <Stable Root operation>' "Tactical Design template should preserve Capability traceability beside the Root method"
assert_not_contains "$tactical_design_template" 'Role->>Interface: Command: <Business intent>' "Tactical Design template should not use business intent as an invocation label"
assert_not_contains "$tactical_design_template" 'Application->>Root: Capability: <Stable Root operation>' "Tactical Design template should not use Capability as a method label"
assert_contains "$tactical_design_template" 'Note over Role,Root: Workshop Event: <Past-tense business outcome>' "Tactical Design template should render analytical outcomes as notes"
assert_contains "$tactical_design_template" 'Published Fact Contract, Integration Message' "Tactical Design template should keep cross-context fact and wire concepts separate"
assert_contains "$tactical_design_template" '## Tactical Design Claims' "Tactical Design template should expose immutable claim IDs"
assert_contains "$tactical_design_template" '| Claim ID | Responsibility | Accepted assertion |' "Tactical Design claims should be compact and reviewable"
assert_contains "$tactical_design_template" '<a id="TD-001"></a>TD-001' "Tactical Design claim IDs should expose stable fragment anchors"
assert_contains "$tactical_design_template" '## BC Architecture Projection' "Tactical Design should expose the exact current-authority projection for confirmation"
assert_contains "$tactical_design_template" 'Each Tactical Design Claim appears exactly once' "Tactical Design template should require exhaustive Architecture disposition"
assert_contains "$tactical_design_template" '| Source claim | Disposition | Bounded Context | Action | Decision ID | Current BC-specific architecture decision or iteration-only reason |' "Tactical Design template should capture projected and iteration-only claims in one ledger"
assert_contains "$tactical_design_template" '`iteration-only` requires a concrete reason' "Tactical Design template should explain every omitted Architecture projection"
assert_not_contains "$tactical_design_template" 'Omit this section when no accepted claim must survive as a current BC-specific architecture decision' "Tactical Design should not silently omit Architecture disposition"
assert_contains "$tactical_design_template" '## Codify Discretion' "Tactical Design should leave reversible choices to Codify"
...
assert_contains "$claude_maintainer" 'every BC-owned technical object to appear inside exactly one Mermaid `box ... end` for its Bounded Context' "tactical proposal validation should require Bounded Context grouping"
assert_contains "$claude_maintainer" "every invocation arrow into or between technical participants to name the receiver's \`Public Method\`" "tactical proposal validation should require receiver Public Methods"
...
assert_contains "$claude_maintainer" 'account every Tactical Design Claim exactly once as either `projected` or `iteration-only`' "tactical proposal validation should reject unaccounted claims"
assert_contains "$claude_maintainer" 'every projected `add` or `replace` claim has one matching supplied current Architecture row' "tactical proposal validation should bind durable additions and replacements to rendered Architecture"
assert_contains "$claude_maintainer" 'every projected `remove` claim has one matching supplied removal' "tactical proposal validation should bind durable removals to observed Architecture"
...
assert_contains "$claude_maintainer" 'do not invalidate an existing `ready` or `implemented` Tactical Design solely because its sequence layout or labels predate the current Bounded Context box and Public Method projection form' "artifact inspection should migrate Tactical Design sequence formats lazily"
assert_contains "$claude_maintainer" 'claim disposition predates the exhaustive Architecture-disposition form' "artifact inspection should preserve pre-contract Tactical Design history"
...
assert_contains "$CLAUDE_ROOT/README.md" 'every Tactical Design Claim is accounted for as `projected` or `iteration-only`' "Claude README should expose exhaustive Architecture disposition"
assert_contains "$CODEX_ROOT/README.md" 'every Tactical Design Claim is accounted for as `projected` or `iteration-only`' "Codex README should expose exhaustive Architecture disposition"
...
assert_contains "$ROOT/README.md" 'every Tactical Design claim is explicitly classified as durable or iteration-only' "root README should expose exhaustive Architecture disposition"
...
assert_contains "$tactical_design_adr" 'group every technical object by its owning Bounded Context in one Mermaid `box ... end`' "Tactical Design ADR should require Bounded Context object grouping"
assert_contains "$tactical_design_adr" "Every invocation arrow into or between technical participants names the receiver's \`Public Method\`" "Tactical Design ADR should require receiver Public Methods"
