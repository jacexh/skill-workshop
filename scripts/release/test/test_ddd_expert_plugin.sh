...
assert_contains "$event_storming_skill" 'connected `actor/external -> Command -> policy/rule -> Aggregate Capability or explicit coordination -> Workshop Event` thread' "EventStorming should connect normalized intent to Root behavior in every causal thread"
...
assert_contains "$CLAUDE_ROOT/templates/event-storming.md" '| Bounded Context | Aggregate Root | Source Command(s) | Capability | Required authoritative facts | Guaranteed business result | Stable rejection |' "EventStorming minutes should make Command-to-Capability projection checkable"
assert_contains "$CLAUDE_ROOT/templates/event-storming.md" 'one row per stable Root operation rather than per pre-state or scenario branch' "EventStorming minutes should keep capability granularity method-shaped"
