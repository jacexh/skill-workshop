...
assert_contains "$tactical_design_skill" 'Name the behavior with an intention-revealing Domain verb phrase, not a technical implementation action.' "Behavior names should use Domain verbs"
...
assert_contains "$domain_objects_template" '<Subject> <domain verb> <Object>[, transitioning <Lifecycle State name> from <before> to <after>].' "domain-object behavior should use a Domain action and optional Lifecycle State transition"
