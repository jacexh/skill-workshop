...
check_review_evidence_gate() {
  ...
  grep -q "Positive notes are only for surfaces that were not smell rows" "$review_skill" || fail "$label review should keep smell rows out of positive notes"
  ...
  grep -q "No-finding notes are only for surfaces outside required family rows that were not smell rows and have observed correct shape" "$review_skill" || fail "$label review output should keep required rows out of no-finding notes"
  grep -q "Required family rows never go to No-Finding Notes" "$review_skill" || fail "$label review output should forbid required row no-finding notes"
