...
assert_contains "$tactical_design_skill" 'Use depth-first order: Aggregate Root A, every Entity inside A, then Aggregate Root B' "Tactical Design should finish one Aggregate before opening the next"
assert_contains "$tactical_design_skill" 'definition, state, behavior, and actual Domain Events' "Tactical Design should stay at low-resolution object design"
assert_contains "$tactical_design_skill" 'subject, action, object, and result' "Tactical behavior descriptions should carry a complete semantic sentence"
assert_contains "$tactical_design_skill" 'The described Domain object is the grammatical subject and behavior owner' "Tactical Design should bind behavior to an object"
...
assert_contains "$tactical_design_skill" 'Only discuss transaction, concurrency, recovery, or call direction when a confirmed requirement makes it material' "Tactical Design should avoid speculative system complexity"
...
assert_contains "$ROOT/README.md" 'depth-first through one Aggregate Root and all of its Entities' "root README should expose Tactical Design order"
...
assert_contains "$sparse_adr" 'Aggregate Root A -> Entity B -> Entity C -> Aggregate Root B' "ADR should define depth-first Tactical Design"
...
assert_contains "$ROOT/CONTEXT.md" '**Depth-first Tactical Design**:' "shared vocabulary should define discussion order"
...
assert_contains "$ROOT/CONTEXT.md" 'It normally maps to a method on that object' "shared vocabulary should bind behavior to methods"
