...
ddd_collaboration_reference="$CLAUDE_ROOT/references/ddd-collaboration.md"
...
assert_contains "$event_storming_skill" 'even when nobody has proposed an event' "EventStorming should preserve causal pairs before an event candidate exists"
assert_contains "$event_storming_skill" 'whether the first occurrence is already a successful business fact' "EventStorming should expose continuation success semantics"
assert_contains "$event_storming_skill" 'preserve whether the first occurrence stands as a successful fact without that later outcome' "EventStorming should persist continuation success semantics as a Business Rule"
...
assert_contains "$tactical_design_skill" 'material candidate occurrence and required next intent' "Tactical Design should not assume a causal fact before classification"
...
assert_contains "$tactical_design_skill" '<Behavior> would establish <candidate past-tense fact>; <owner> must or may <next intent>.' "Tactical Design should not establish a candidate fact before classification"
assert_contains "$tactical_design_skill" 'Classify it as one success guarantee, an independent local reaction, a cross-context published fact, or no selected event' "Tactical Design should classify every material continuation"
assert_contains "$tactical_design_skill" 'under one success guarantee it exists only after the composed required outcome succeeds' "Tactical Design should establish facts at the accepted success boundary"
assert_contains "$tactical_design_skill" 'recommend the event as the causal handoff to the separately owned next intent' "Tactical Design should use a selected local event to decouple the reaction"
assert_contains "$tactical_design_skill" 'do not retain a direct invocation from the producing behavior as the primary or parallel path' "Tactical Design should reject an event beside the original coupled path"
assert_contains "$tactical_design_skill" 'Apply the durable-occurrence evidence test independently rather than using it to split a success guarantee' "Tactical Design should keep evidence and reaction dimensions distinct"
assert_contains "$tactical_design_skill" 'Being inside one Aggregate or Bounded Context does not decide the classification' "Tactical Design should not conflate consistency membership with direct composition"
assert_contains "$tactical_design_skill" 'every material causal continuation has an accepted classification' "Tactical Design should make continuation coverage a Root completion condition"
assert_contains "$ddd_collaboration_reference" '### Reaction Probe' "collaboration rules should own the normative continuation probe"
for outcome in 'One success guarantee' 'Independent local reaction' 'Cross-context published fact' 'No selected event'; do
  assert_contains "$ddd_collaboration_reference" "$outcome" "Reaction Probe missing $outcome outcome"
done
assert_contains "$ddd_collaboration_reference" 'Aggregate and Bounded Context membership do not decide this classification' "Reaction Probe should classify by guarantees rather than containment"
assert_contains "$ddd_collaboration_reference" 'Do not claim that the occurrence is an independently established fact until the relationship is classified' "Reaction Probe should not invent a premature fact"
assert_contains "$ddd_collaboration_reference" 'Apply the durable-occurrence part of the Domain Event selection predicate independently' "Reaction Probe should keep durable evidence independent from continuation success"
...
assert_contains "$tactical_design_skill" 'no selection reason or consumer' "Domain Event entries should remain producer-only"
...
assert_contains "$codify_skill" 'That latitude does not include inventing the business relationship' "Codify should distinguish structure latitude from missing collaboration semantics"
assert_contains "$codify_skill" 'state that exact semantic gap and leave the path unchanged' "Codify should not implement through missing collaboration authority"
assert_contains "$codify_skill" 'a software mechanism cannot supply the decision' "Codify should not let callbacks or dispatchers choose Domain semantics"
assert_contains "$codify_skill" 'realize the recorded Domain Event as its causal handoff' "Codify should let accepted local events drive their reactions"
assert_contains "$codify_skill" 'observational duplicate' "Codify should remove direct paths that bypass accepted event causality"
...
assert_contains "$guard_skill" 'unclassified business continuation' "Guard should identify unsupported continuation semantics"
assert_contains "$guard_skill" 'do not report a missing Domain Event merely because a follow-up exists' "Guard should not demand events indiscriminately"
assert_contains "$guard_skill" 'the event is the causal handoff to the next intent' "Guard should verify accepted event causality"
assert_contains "$guard_skill" 'observational event beside the original coupled path' "Guard should find event-plus-direct-path drift"
...
continuation_adr="$ROOT/docs/adr/0010-classify-material-causal-continuations.md"
[ -f "$continuation_adr" ] || fail "causal continuation ADR missing"
assert_contains "$continuation_adr" 'Status: Accepted' "causal continuation ADR should be accepted"
assert_contains "$continuation_adr" 'even when neither the user nor the model mentions an event' "ADR should make event candidate generation active"
assert_contains "$continuation_adr" 'one success guarantee' "ADR should retain the direct-composition counterexample"
assert_contains "$continuation_adr" 'The sparse artifact contract does not change' "ADR should preserve sparse current artifacts"
...
assert_contains "$ROOT/CONTEXT.md" '**Material Causal Continuation**:' "shared vocabulary should name the continuation under classification"
assert_contains "$ROOT/CONTEXT.md" 'classifies every Material Causal Continuation before realization' "shared vocabulary should make the probe part of Tactical Design"
