...
assert_contains "$event_storming_skill" 'selected Domain Event or Published Fact Contract -> Reaction Policy -> Command' "EventStorming should preserve required event-to-intent causality"
assert_contains "$event_storming_skill" 'decision rule admits or rejects one Command; a Reaction Policy observes an established fact and issues the next intent' "EventStorming should distinguish command decisions from event reactions"
assert_contains "$event_storming_skill" 'Persist only business-required reactions' "EventStorming should not promote implementation-only event wiring into the Model"
...
assert_contains "$CLAUDE_ROOT/references/ddd-modeling.md" 'Each supported Aggregate Root exposes intention-revealing business capabilities' "DDD modeling guidance should assign behavior to roots"
assert_contains "$CLAUDE_ROOT/references/ddd-modeling.md" 'A capability usually maps to a Domain method but is not a method signature' "DDD modeling guidance should keep capabilities business-facing"
...
assert_contains "$CLAUDE_ROOT/templates/event-storming.md" '## Aggregate Capabilities' "EventStorming minutes should expose the exact capability projection being confirmed"
assert_contains "$CLAUDE_ROOT/templates/event-storming.md" '| Bounded Context | Aggregate Root | Capability | Business intent | Required facts | State transition or outcome | Rejection or failure |' "EventStorming minutes should make cross-context capability confirmation checkable"
assert_contains "$CLAUDE_ROOT/templates/event-storming.md" '## Required Reactions' "EventStorming minutes should expose only durable event-to-intent projections"
assert_contains "$CLAUDE_ROOT/templates/event-storming.md" '| Reacting Bounded Context | Observed Domain Event or Published Fact Contract | Reaction Policy | Owner | Issued Command | Target Aggregate Capability or coordination | Business failure or recovery |' "EventStorming minutes should make required reactions confirmable"
assert_contains "$CLAUDE_ROOT/templates/event-storming.md" 'Omit this section when the confirmed Model requires no reaction' "EventStorming minutes should not force an empty reaction catalog"
...
assert_contains "$CLAUDE_ROOT/templates/model.md" '| Capability | Business intent | Required facts | State transition or outcome | Rejection or failure |' "model template should make Aggregate capabilities checkable"
assert_contains "$CLAUDE_ROOT/templates/model.md" 'usually maps to an intention-revealing method' "model template should connect business capability to code without prescribing signatures"
assert_contains "$event_storming_skill" 'Account for every state-changing Command or policy decision through one or more owned Aggregate Root capabilities or explicit Application/cross-Aggregate coordination' "EventStorming step eight should close behavior ownership gaps without forcing one Root"
assert_contains "$event_storming_skill" 'Do not prescribe method signatures, fields, packages, or persistence' "EventStorming should keep Aggregate capabilities at business-authority level"
...
assert_contains "$CLAUDE_ROOT/templates/model.md" '### Required Reactions' "model template should retain business-required event-to-intent causality"
assert_contains "$CLAUDE_ROOT/templates/model.md" '| Observed Domain Event or Published Fact Contract | Reaction Policy and owner | Issued Command | Target Aggregate Capability or coordination | Business failure or recovery |' "model template should connect facts to owned Aggregate behavior"
assert_contains "$CLAUDE_ROOT/templates/model.md" 'Omit this section when the confirmed Model selects no Domain Events and requires no reactions' "model template should not force an empty event or reaction catalog"
...
assert_contains "$CLAUDE_ROOT/templates/model.md" 'Do not prescribe handlers, dispatch, transport, transactions, or retry mechanisms' "model reactions should remain business authority rather than software wiring"
...
assert_contains "$tactical_design_skill" 'complete critical collaboration sequence' "Tactical Design should close the coordination gap with complete sequence diagrams"
...
assert_contains "$tactical_design_skill" 'Return one **Model Challenge**' "Tactical Design should hand business-model counterexamples back explicitly"
assert_contains "$tactical_design_skill" 'conversation handback, not another project artifact' "Tactical Design should connect workflows without creating handoff-file sprawl"
...
assert_contains "$tactical_design_skill" 'challenged Model path, revision, and section' "Tactical Design should identify the exact business authority under challenge"
assert_contains "$tactical_design_skill" 'business scenario or counterexample' "Tactical Design should hand EventStorming falsifiable evidence"
assert_contains "$tactical_design_skill" 'leave the Tactical Design draft byte-identical' "Tactical Design should freeze design while business authority is reopened"
assert_contains "$tactical_design_skill" 'A challenge starts or extends one revision batch; it never triggers an immediate draft write' "Tactical Design should clarify a connected concern before redrawing"
...
assert_contains "$tactical_design_skill" 'rebase the same draft as one complete revision batch' "Tactical Design should resume coherently after EventStorming changes"
...
assert_contains "$tactical_design_skill" '`awaiting_confirmation`: no frontier question or revision batch remains' "Tactical Design should reserve final confirmation for a settled draft"
...
assert_contains "$CLAUDE_ROOT/skills/guard/SKILL.md" 'Do not parse every Mermaid arrow' "guard should judge claims instead of mechanically auditing diagrams"
...
assert_contains "$ROOT/README.md" 'Tactical Design confirms complete critical sequences' "root README should expose the collaboration-design bridge"
...
assert_contains "$tactical_design_adr" '`model.md` owns current Bounded Context business authority, Aggregate Capabilities, and Required Reactions' "Tactical Design ADR should preserve Model ownership"
...
assert_contains "$tactical_design_adr" 'one complete critical sequence per material success, failure, or recovery path' "Tactical Design ADR should require complete collaboration views"
...
assert_contains "$tactical_design_adr" 'one connected revision batch' "Tactical Design ADR should batch related clarification before redrawing"
...
assert_contains "$ROOT/CONTEXT.md" '**Required Reaction**:' "shared vocabulary should define business-required event-to-intent causality"
