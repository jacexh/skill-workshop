...
check_review_evidence_gate() {
  ...
  grep -q "\\*\\*First-hop completion\\*\\*: add visible domain state/event vocabulary" "$review_skill" || fail "$label review workflow should complete first-hop smells"
  grep -q "Output: completed first-hop Smell List" "$review_skill" || fail "$label review workflow should output completed first-hop smell list"
  grep -q "\\*\\*Merge\\*\\*: group same-shape smells by owner, lifecycle, boundary, or mechanism\\. Output: merged smell families" "$review_skill" || fail "$label review workflow should output merged smell families"
  grep -q "\\*\\*Explain one family\\*\\*: process one merged smell family" "$review_skill" || fail "$label review workflow should explain one smell family at a time"
  ...
  grep -q "\\*\\*Expand siblings\\*\\*: check sibling methods, flows, states, events, ports, and adapters" "$review_skill" || fail "$label review workflow should expand sibling shapes"
  grep -q "Output: updated family verdict plus any new adjacent smell rows" "$review_skill" || fail "$label review workflow should output updated verdicts and adjacent rows"
  grep -q "\\*\\*Close the list\\*\\*: repeat explain/expand until every smell and adjacent smell has a terminal verdict" "$review_skill" || fail "$label review workflow should close every smell"
  grep -q "Output: closed Smell List" "$review_skill" || fail "$label review workflow should output closed smell list"
  grep -q "triggered first-hop families without method/flow/state/event-level correct-shape evidence become evidence gaps" "$review_skill" || fail "$label review workflow should not clear triggered first-hop families without correct-shape evidence"
  grep -q "\\*\\*Synthesize\\*\\*: combine closed smell verdicts\\. Output: shared wrong model, boundary, lifecycle, mechanism, or missing recovery story" "$review_skill" || fail "$label review workflow should output root synthesis"
  grep -q "\\*\\*Report\\*\\*: turn closed verdicts into findings, evidence gaps / returns, non-smell positive notes, verification, and residual risk\\. Output: final review judgment" "$review_skill" || fail "$label review workflow should output final review judgment"
  ...
  grep -q "smells do not become no-finding" "$review_skill" || fail "$label review should forbid no-finding smell verdicts"
  ...
  grep -q "## No-Finding Admission" "$review_skill" || fail "$label review should define no-finding admission"
  grep -q "No-finding notes are allowed only for non-smell surfaces or triggered families with observed correct shape" "$review_skill" || fail "$label review should require observed correct shape before no-finding notes"
  grep -q "Accepted design, semantic names, object splitting, package separation, DTO/read-model presence, and a passing test are not no-finding proof" "$review_skill" || fail "$label review should reject weak no-finding proof"
  grep -q 'Repository/API no-finding needs a method inventory showing `Get`/`Save` shape' "$review_skill" || fail "$label review should require repository method inventory before no-finding"
  grep -q "Collaboration no-finding needs each delivery/refund/dispute/settlement/recovery flow to name its Domain Event" "$review_skill" || fail "$label review should require collaboration mechanism proof before no-finding"
  grep -q "Terminal/execution no-finding needs separate money execution facts and agreement terminal facts" "$review_skill" || fail "$label review should require terminal/execution proof before no-finding"
  grep -q "CQRS no-finding needs write-repository method inventory plus product-read QueryRepository/read-facade evidence" "$review_skill" || fail "$label review should require CQRS method inventory before no-finding"
  ...
  grep -q "cite smell families in findings, evidence gaps, or returns" "$review_skill" || fail "$label review should cite smell families in decisions"
  grep -q "Every explained smell-family verdict must land in Findings or Evidence gaps / returns" "$review_skill" || fail "$label review should not drop explained smell-family verdicts"
  grep -q "Do not collapse production wiring, collaboration mechanism, candidate-owner, state vocabulary, or CQRS method-inventory decisions into a broader finding" "$review_skill" || fail "$label review should not hide smell-family decisions behind broader findings"
  grep -q "Report in this order when present: scope/model evidence, findings, evidence gaps / returns, no-finding notes for non-smell surfaces with positive shape, verification, residual risk" "$review_skill" || fail "$label review output should define concise report order"
  ! grep -q "depth execution" "$review_skill" || fail "$label review output should not use old depth-execution wording"
  grep -q "No-finding notes must name the observed correct shape" "$review_skill" || fail "$label review output should name observed correct shape for no-finding notes"
  grep -q "if the proof is package names, object splitting, accepted design, DTO presence, or passing tests only, report an evidence gap" "$review_skill" || fail "$label review output should downgrade weak no-finding proof to evidence gap"
...
check_core_default_first_reference() {
  ...
  grep -q 'Repository default: one Repository is a collection for one write-side Aggregate Root, normally with only `Get` and `Save`' "$core" || fail "$label core should default Repository APIs to Get/Save"
  ...
  grep -q "No-finding needs observed correct shape for the touched surface" "$core" || fail "$label core should require observed correct shape for no-finding"
  ...
  grep -q "Positive conclusion calibration" "$core" || fail "$label core should keep post-review calibration rule"
