...
assert_contains "$event_storming_skill" '../../references/ddd-collaboration.md' "EventStorming should load collaboration rules for a candidate Domain Event"
...
assert_contains "$tactical_design_skill" 'When a candidate Domain Event appears, load the Domain Event rules in `ddd-collaboration.md`' "Tactical Design should load the normative event-selection rules for candidates"
...
assert_contains "$codify_skill" 'temporary, task-local trace' "Codify should keep implementation traceability lightweight and transient"
assert_contains "$codify_skill" 'affected implementation paths, and verification evidence' "Codify should connect affected model clauses to realization evidence"
...
assert_contains "$codify_skill" 'Let accepted Domain ownership and behavior drive the affected software structure' "Codify should derive affected software structure from the accepted Domain"
assert_contains "$codify_skill" 'preserve outer structures that support them, reshape those that obscure or duplicate them' "Codify should preserve compatible outer structure and reshape conflicting responsibility"
assert_contains "$codify_skill" 'remaining realization choices that preserve the accepted semantics' "Codify should apply House Style within accepted semantics"
