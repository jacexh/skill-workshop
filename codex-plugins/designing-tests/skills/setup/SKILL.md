...
Do not run this skill for regular upgrades when native plugin hooks work. If this fallback was used previously and native hooks are now enabled, use `$designing-tests:cleanup` once to remove stale fallback entries from `~/.codex/hooks.json`.
...
- Do not use setup as a routine upgrade step on current Codex builds; prefer native hooks plus `$designing-tests:cleanup` for migration off fallback entries.
