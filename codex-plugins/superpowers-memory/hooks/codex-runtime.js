...
const STANDING_PRIMER =
  ...
  "- Before finishing, committing, merging, or opening a PR: inspect stale changes and invoke $superpowers-memory:ingest only for meaningful durable project knowledge changes.\n";
...
// Shared classifier for finishing-a-development-branch, used by PreToolUse
// (Skill tool invocation, Claude only) and UserPromptSubmit (slash-typed mention,
// Codex only). Handles the 4-way staleness classifier (base-branch no-op,
// SHA-match soft reminder, KB-only-commits soft reminder, rich injection fallback).
// eventName is propagated to hookSpecificOutput.hookEventName, so it must match
// the caller's actual event ("PreToolUse" or "UserPromptSubmit").
...
function buildPreToolUseOutput(input) {
  ...
  // Claude Code write tools and Codex apply_patch / mcp__filesystem__.* tools.
