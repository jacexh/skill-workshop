...
Read `docs/superpowers/memory/` to answer a concrete project question or to route follow-up code exploration. This skill is read-only.
...
Do not write knowledge files. If `query` emits a Memory candidate, keep it as
proposed input for a later `ingest`.

**Announce at start:** "I'm querying the project knowledge base."

## Process

1. If `docs/superpowers/memory/` does not exist, tell the user: "Project knowledge base not initialized. Run superpowers-memory:ingest bootstrap to create it."
2. Read `docs/superpowers/memory/index.md`. If it does not exist, fall back to reading the canonical owner files that exist.
3. Run **Question classification** before opening owner files:
...
4. Normalize the question into likely project terms and aliases. Use glossary entries as alias expansion, not as a default context dump.
5. Build a **Retrieval route** before reading details:
...
6. Select the smallest useful set of owner files or shards, normally 1-3.
7. If the index is insufficient, search `docs/superpowers/memory/` for terms and aliases. Prefer targeted `rg` hits over opening a large root file.
8. Read candidate owner entries.
9. Follow `See:`, `Related:`, ADR, spec, plan, or source references only when evidence is not yet sufficient. Default to one hop; take a second hop only for contradiction, low confidence, or missing source refs.
10. Stop when at least one owner entry answers the question or when the route has produced enough code search seeds for the next step. Linked references must not contradict the answer.
11. If no owner entry directly answers a durable project question, answer with low confidence from the best evidence and emit a structured Memory candidate for `ingest`.
12. If the answer creates a durable synthesis, comparison, or analysis that is likely to be reused, emit a structured Memory candidate even when the current answer is complete.
13. conversation is not a KB slot: do not suggest `conversation.md`. If the reusable fact came from chat/transcript context, route the Memory candidate to a spec/plan/ADR first when appropriate, or to the existing owner file that owns the durable fact.
