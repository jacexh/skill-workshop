...
Files outside the mapping are out of scope — for example, `adr/ADR-NNN-<slug>.md` detail files are not scoped targets (they're rebuilt as part of `decisions.md`, or authored directly during `update`).
...
3. **Per-file format rules** (from `content-rules.md`):
   ...
   - `features.md`: current capability map — `##` lifecycle state, `###` capability group, `####` capability entry with fixed fields (`Enables`, `Actors / Entry Points`, `Capability Boundary`, `References`). No long single-paragraph entries, SHAs, test counts, or changelog narrative.
...
For each of the 6 canonical knowledge files (full mode) or the single target file (scoped mode), use the plugin template as the structural basis and fill in concrete content from the codebase analysis. If a valid non-index file becomes too large to scan, split it by stable domain/submodule into `<slot>-<domain>.md` and link it from the entry file plus `index.md`.
...
- Re-read all existing knowledge files (including the one(s) just rewritten). Include recognized shard files (`architecture-*.md`, `features-*.md`, `conventions-*.md`, `tech-stack-*.md`, `decisions-*.md`, `glossary-*.md`) when they exist; do not add placeholder lines for omitted or legacy files.
...
**Hot-path constraint:** Keep index.md under 50 lines total.
...
Staging the directory root recursively picks up everything underneath (`adr/`, shard files, all KB files). Do not list subdirectories explicitly — passing a literal pathspec that doesn't exist makes `git add` exit with `fatal: pathspec ... did not match any files`.
...
Follow all rules defined in [`content-rules.md`](../../content-rules.md) — language, inclusion/exclusion, SSOT, quality, progressive knowledge layout, and retrieval-cost guidance.
