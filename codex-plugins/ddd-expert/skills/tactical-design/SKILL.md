...
2. **Identify collaborating roles**: start from the confirmed Command-to-Capability mappings and name only the Domain owners, Application orchestrators, semantic ports, Repositories, external authorities, and Runtime composition needed to explain the material collaboration. A participant is a responsibility, not necessarily a class or package.
3. **Draw complete critical collaboration sequences**: use Mermaid `sequenceDiagram`. For each material success, rejection, failure, timeout, retry, or recovery path, show the initiating actor and normalized Command, Application coordination, exact accepted Aggregate Capability, authoritative reads/writes, transaction boundary, state/checkpoint changes, event publication timing, and externally visible outcome. Reuse one capability across its state variants; every additional Domain call must remain traceable to that capability or an accepted Model policy rather than becoming a new state-qualified operation. Omit mechanically identical variants, but never stop a sequence at the interesting middle.
...
5. **Define changed Interfaces and seams**: state only new or changed semantic capabilities between responsibilities. Use business language rather than provider, table, protocol, topology, or SDK mechanisms. Connect each Aggregate call to an accepted Aggregate Capability or Model policy, and do not make Application select state-qualified Root operations that duplicate a decision owned by the Aggregate.
