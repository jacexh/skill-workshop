...
Collaboration policy inventory: Collaboration rows must include payment, delivery, refund, dispute, settlement, split closure, and every accepted lifecycle reaction.
...
Stale-command rights matrix rows must contain exactly one durable fact and one later command; grouped command cells such as cancel/retry/fund are invalid.
