...
If the expected bounded context, data authority, invariant owner, layer owner, or local convention cannot be reconstructed, report an evidence gap instead of inventing a model.
...
4. Decide each candidate as `Rules Satisfied / Not Applicable / Exception / Evidence gap`.
