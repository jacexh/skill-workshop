...
Final-output checkpoint: after the first model/spec/diff inventory batch, emit the DDD review from the current axis ledger. Do not start a second broad evidence batch before final output. Never end a review turn after tool output with reasoning only; the next assistant-visible message must be final DDD review output or one explicitly row-local tool call.
