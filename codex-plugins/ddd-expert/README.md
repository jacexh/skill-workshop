...
Use EventStorming as the single modeling path from backend evidence to a codify-ready design. This plugin is hookless; automatic intervention relies on each skill's own frontmatter description.
...
- **`$ddd-expert:event-storming` skill** — Continuous workflow from backend stories or specifications through proportionate Big Picture and Process Modelling, accepted Models and Context Map, Software Design EventStorming, Tactical Design, visible readiness, and a codify-ready handoff
...
The plugin does not auto-inject context. EventStorming is the single modeling workflow; Codify and Guard remain focused explicit entries. The plugin also ships `maintain-artifacts` as an internal protocol, not a user entry point.
