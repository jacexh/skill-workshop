...
| Aggregate Boundary Conflict | `ddd-modeling-gates.md`, `ddd-core.md`, active language Domain/Infrastructure guide | Repository/API saves or coordinates several candidate roots/lifecycle owners; implementation transaction evidence; owned-child evidence; event-driven coordination evidence; red-flag evidence such as semantic repository transaction, lifecycle transaction, cross-table transaction, same persistence boundary, or ORM session | Default rule: one Repository saves one Aggregate Root; implementation transaction evidence is not model evidence; transaction-shaped evidence cannot satisfy Repository design; return to domain-modeling |
