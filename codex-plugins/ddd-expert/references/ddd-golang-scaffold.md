...
proto/
  <contract-owner>/
    private/v1/                 # deployment-private RPC contracts
    public/v1/                  # externally supported RPC contracts
    integration/v1/             # Integration Messages, when present
    task/v1/                    # durable Task payload schemas, when present
    ...
    <context-a>/
      ...
      application/
        ...
        task/                     # only when background execution is required
          <task>.go               # TaskType, Definition, constructor
...
- Deployment-private RPC source lives under `proto/<context>/private/v1`; externally supported RPC source lives under `proto/<context>/public/v1`. ConnectRPC is the transport for either surface, not a directory category. Generated Go and Connect stubs mirror the selected path under `gen/`.
- Use `private`, not a path segment named `internal`, so source-relative Go generation does not trigger Go's `internal` import restriction.
...
- Durable Task payload schemas live under `proto/<semantic-owner>/task/v1`; generated code mirrors that path under `gen/`. `application/task` owns the semantic `TaskType`, `Definition`, and protobuf-backed constructor; a Task remains internal execution work, not a cross-context API.
- Domain never imports `gen`. Producing Application may import only its own generated Integration Message or Task payload contract. Transport may import generated inbound RPC/message/task contracts. Infrastructure may import generated external client contracts for an ACL.
