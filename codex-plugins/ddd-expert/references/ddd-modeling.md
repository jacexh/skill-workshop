...
- **[House Rule]** Normalize Commands by the intent chosen by the actor or issuing policy. Reuse one canonical Command across scenario and pre-state variants when the requested business outcome is the same; a modeler-added state adjective does not create another Command.
...
- **[House Rule]** A capability is one stable business operation exposed by the Root. Do not split it by pre-state, success branch, rejection branch, timing case, or other scenario variation when the caller still expresses one intent. Split only when the actor or issuing policy chooses a materially different business intent or commitment.
- **[House Rule]** Record only the capability's common required authoritative facts, guaranteed business result, and stable rejection contract. Keep branch logic in scenario threads, lifecycle, invariants, and policies; keep Application or cross-Aggregate sequencing out of the capability row.
