...
- **[Heuristic]** Order material events in business time, then work backward to Commands and initiating Roles or external authorities and forward through event-triggered intents and stable outcomes.
- **[DDD Principle]** A Command expresses a business intent; an Event states the resulting fact. Constraints explain whether the owning business authority may establish that fact.
- **[House Rule]** A **Role** is a named business participant whose decision rights are defined in one Bounded Context. Place it inside that context and outside every Aggregate; do not substitute an IAM principal, transport identity, or generic Actor label.
- **[House Rule]** Normalize Commands by the intent chosen by the initiating Role or required by an established fact. Reuse one canonical Command across scenario and pre-state variants when the requested business outcome is the same; a modeler-added state adjective does not create another Command.
- **[House Rule]** In the integrated EventStorming diagram, express a Command as the labeled arrow from its initiating Role, selected Domain Event, Published Fact Contract, or external authority to the target Aggregate Capability or explicit coordination. Keep the Command as semantic traceability, not a second node beside a same-named Capability.
- **[House Rule]** Persist a business-required `selected Domain Event or Published Fact Contract -- Command --> Aggregate Capability or explicit coordination` mapping in the reacting context's current Model. The reacting context owns the Command; keep implementation-only callback, handler, transport, and delivery wiring out of the Model.
...
- **[Heuristic]** Replay failure, duplication, cancellation, expiry, recovery, and concurrency on the Board when they could change business meaning. Keep generic rejection, permission, validation, and unchanged-result branches out of the integrated diagram; retain an adverse Workshop Event only when the established fact itself changes rights, obligations, value, or a required next action.
...
- **[House Rule]** A capability is one stable business operation exposed by the Root. Do not split it by pre-state, success branch, rejection branch, timing case, or other scenario variation when the caller still expresses one intent. Split only when an initiating Role chooses, or an established fact requires, a materially different business intent or commitment.
- **[House Rule]** Record only the capability's common required authoritative facts, guaranteed business result, and stable rejection contract. Keep branch logic in lifecycle, invariants, and policies; keep generic failed branches out of the integrated diagram and Application or cross-Aggregate sequencing out of the capability row.
- **[House Rule]** A capability guides the Root's intention-revealing Domain method surface but is not a signature or a method-count rule. One implementation may use internal branches, helpers, or a justified prepare/complete protocol; do not make callers choose state-qualified operations by reproducing the Root's decision outside it.
