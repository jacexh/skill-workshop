...
- Counterfactual Review Gateway: A checked row is provisional until it survives falsification; draft findings are not final. Before final output, ask what evidence would make each checked/Rules Satisfied row false across durable fact precedence, owner proof, reaction/recovery reachability, state language, and CQRS read/write semantics.
