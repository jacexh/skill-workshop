...
24. **Periodic scheduler callbacks.** Do not add scheduler `HandleFunc`, callback registration, or business-service calls for periodic taskqueue work. Periodic producers enqueue concrete `taskqueue.PeriodicTask` values; the normal one-`TaskType`, one-`Processor` task contract handles execution. (taskqueue §5)
...
- [ ] **Periodic producer explicit**: recurring work uses `taskqueue.PeriodicTask`, `CronSchedule` / `IntervalSchedule`, and `PeriodicTaskScheduler`; the scheduler enqueues static task envelopes and does not expose `HandleFunc` or business callbacks.
