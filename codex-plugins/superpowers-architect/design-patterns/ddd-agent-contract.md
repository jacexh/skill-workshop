...
- [ ] **Periodic producer explicit**: recurring work uses `taskqueue.PeriodicTask`, `CronSchedule` / `IntervalSchedule`, and `PeriodicTaskScheduler`; the scheduler enqueues static task envelopes and does not expose `HandleFunc` or business callbacks. Disabled periodic tasks are omitted from registration, `WithLocation` is cron-only, periodic policies avoid `WithProcessAt` / `WithDeadline`, and duplicate-name guarantees are scoped to one scheduler/registrar instance unless runtime coordination is documented.
