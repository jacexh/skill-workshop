...
If the user says something like "dogfood vercel.com", start immediately with defaults. Ask only for missing information needed to proceed, such as the target URL or access to an authenticated test account.

This skill requires the `agent-browser` CLI and its browser runtime; they are not bundled with the plugin. Check `agent-browser --version` and `agent-browser --help` before starting. If unavailable, report the missing dependency and refer to the plugin README; do not claim that browser testing ran. Use the installed `agent-browser` directly.

Resolve `{SKILL_DIR}` to the directory containing this file. Replace all command placeholders before running them, and quote URLs and filesystem paths.

Keep exploration within the requested scope and existing authorization. Use disposable test records for create/edit/delete workflows. Sending messages, making purchases, or changing real user data requires authorization for that action; a general QA request does not grant it. Capture evidence without exposing credentials or sensitive personal data.
...
Use a fresh run subdirectory if the output directory already contains a report; preserve earlier evidence. Copy the report template into the output directory and fill in the header fields:
...
After navigation, wait for the element, text, or URL needed for the next action. Use `networkidle` only for pages known to become quiet; polling or streaming may keep it pending.
...
Keep authentication in the active session by default. Save reusable login state only when requested, to a private path outside the report directory with restrictive file permissions. Never include credentials, cookies, or authentication state in the shareable report.
...
These require user interaction to reproduce -- use video and step-by-step screenshots. If recording is unavailable, retain screenshots and exact steps, and state the video limitation rather than inventing or discarding evidence:
...
Wrap up when the requested scope or time budget is covered, or when access or tooling blocks further exploration. Report only observed issues; zero findings is a valid result. Record tested areas, skipped areas, and evidence limitations so the report does not imply complete coverage.

After exploring:

1. Re-read the report and update the summary severity counts so they match the actual issues. Count confirmed issues only. Remove the unused example issue block when there are no findings. Keep unconfirmed observations outside severity totals.
2. Close the session:
...
- **Repro is everything.** Every issue needs proof -- but match the evidence to the issue. Interactive bugs should have video and step-by-step screenshots; disclose unavailable recording. Static bugs (typos, placeholder text, visual glitches visible on load) only need a single annotated screenshot.
- **Verify reproducibility before collecting evidence.** Before recording video or taking screenshots, verify the issue is reproducible with at least one retry. If it cannot be reproduced consistently, retain the observation, captured evidence, and retry outcome in an unconfirmed observations section; do not count it as a confirmed issue.
...
- **Never delete output files.** Do not `rm` screenshots, videos, or the report mid-session. Preserve evidence before any necessary recovery or session restart, and note interruptions in the report.
