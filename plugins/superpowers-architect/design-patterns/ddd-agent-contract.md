...
- The task mentions or edits Domain Events, `event.Collection`, Domain Event Handlers, Boundary Publishers, Integration Messages, `message.Publisher`, `message.Handler`, `message.Subscriber`, Kafka message adapters, `application/eventhandler`, `application/messagepublisher`, or `application/messagehandler`.
...
- The task mentions or edits task queues, polling tasks, reconciliation jobs, scheduled/background jobs, asynq, `TaskType`, task payload schemas, task processors, `internal/pkg/taskqueue`, worker lifecycle, task middleware, delayed enqueueing, or schema registry wiring.
...
2. Classify the task path           (DDD/business, Go events/messages, Go taskqueue/polling, Go runtime-only, or mixed)
...
- For **Go event/message work** (Domain Events, event handlers, Boundary Publishers, Integration Messages, message handlers, Kafka message adapters) → read `ddd-golang-events-messages.md` in addition to the matching DDD/business row above. If the work only changes shared eventbus/message adapter runtime wiring, also read `ddd-golang-runtime.md` and state lifecycle/config/shutdown impact.
...
- For **Go taskqueue / polling / asynq work** → read `ddd-golang-taskqueue.md` and `ddd-golang-runtime.md`. If the change adds or modifies task processors, task payloads, enqueueing decisions, or polling policy inside a bounded context, also follow the matching DDD/business row above and emit the Architecture Gate for the business side. If it only changes `internal/pkg/taskqueue` runtime wiring, do not emit the DDD Architecture Gate; state the taskqueue component, ownership, lifecycle hook, config, schema/processor registration impact, and shutdown impact.
...
8. **Drain Domain Events inside the Repository.** Only the Application layer calls `aggregate.Events.Drain()` / `collect_events()`, and only once, after a successful `Save()`. (core §3.1, golang §3.1)
...
10. **Mutate Aggregate state by exported-field assignment.** All state changes go through Aggregate Root methods that enforce invariants. Anemic aggregates (only getters/setters with rules in handlers) are rejected. (core §3.1, golang §3.1)
...
- [ ] **Taskqueue guide read when applicable**: `ddd-golang-taskqueue.md` was read for polling jobs, task processors, schema registry, asynq worker wiring, task middleware, and `internal/pkg/taskqueue`.
...
Use this checklist when editing task queues, polling/reconciliation jobs, asynq wiring, `TaskType`, task payload schemas, task processors, enqueueing, or `internal/pkg/taskqueue`:
...
- [ ] **Runtime ownership**: `internal/pkg/taskqueue` owns asynq client/worker creation, common middleware, processor registration, lifecycle hooks, and graceful shutdown; `cmd/**/main.go` remains a thin entry point.
...
Task classification:
- <DDD/business | Go events/messages | Go taskqueue/polling | Go runtime-only | mixed>
...
| Go taskqueue, polling jobs, asynq workers, task schemas | [`ddd-golang-taskqueue.md`](ddd-golang-taskqueue.md) |
