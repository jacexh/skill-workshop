...
3. **Explain each family**: assume the smell is wrong until the relevant method, flow, state, event, port, or adapter shows the correct shape. Evidence-gap is for missing proof; if inspected evidence shows only a forbidden shape or no required mechanism, output violation, return-to-domain-modeling, or return-to-design instead of pressure.
...
- Collaboration mechanism: command transaction, synchronous save, or repository call is coordination evidence, not the mechanism; repeated cross-lifecycle reactions require a named Domain Event, process manager, reconciler, task processor, Integration Message, explicit operator command, or return/evidence gap; adjacent findings do not clear this row.
...
- Negative evidence upgrade: actual evidence of command-transaction-only coordination, write-side read helpers, parent states named after child outcomes, or missing runtime/collaboration mechanism is a final verdict input, not mere pressure or inventory.
- Required family rows: lifecycle/repository/event/CQRS scope keeps durable-fact command admission, terminal/execution split, repository/API candidate-owner, collaboration mechanism, parent-state vocabulary, accepted-design waiver, and CQRS inventory as separate rows when their evidence is present; one broader finding cannot clear another row.
...
- CQRS inventory: inspect write repositories and shared adapters for list/detail/history/summary/read-shaped methods before clearing CQRS shape; write-side read helpers are negative evidence until caller semantics and returned model family prove command-side aggregate fact lookup; QueryRepository/read-facade presence proves only the read side exists.
- Parent-state vocabulary: parent states named with child execution or attempt outcomes require a parent lifecycle fact; command-admission, recovery, or terminal-event findings do not clear this row.
...
Pressure, inventory, and command-side lookup are intermediate review evidence. When they show an actual forbidden shape or absent required mechanism, upgrade them to a finding or return instead of leaving them as a weak evidence gap.
