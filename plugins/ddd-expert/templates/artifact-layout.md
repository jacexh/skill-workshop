...
- `draft`: the current candidate is visible for confirmation; canonical Models remain unchanged.
...
A Tactical Design record normally follows `draft -> ready -> implemented`; invalidation before implementation adds `ready -> superseded`. `draft` exposes one coherent collaboration candidate that Codify may test through explicitly authorized reversible exploration. The initial draft contains the common thesis and deliberately omits final claims, Architecture disposition, and Reconciliation Evidence. Design-only work stops there. `ready` means Tactical Design reconciled the current candidate with concrete implementation evidence, added the final claims and dispositions, and the user confirmed it; only that state enters final Codify and Guard. `implemented` means Guard cleared the matching realization with a complete producer checkpoint, and `superseded` is invalidated pre-implementation history. A superseded record links through `superseded_by` to either its replacement ready Tactical Design or the ready EventStorming authority left sufficient after the delta disappears.

When newer confirmed Models or concrete implementation evidence under the same current Model eliminate the Design Delta or materially replace its implementation-slice identity, remove an unconfirmed Tactical Design draft and its README entry instead of retaining cancelled design history. Never discard a confirmed record. Replace an invalidated unimplemented `ready` record with a new confirmed Tactical Design when a material delta remains, or retire it directly when no replacement delta remains; in either case transition it to `superseded`, replace its README TODO with plain lineage, and replace or remove every current BC Architecture source from its claims in the same consistency write. Same-authority replacement or retirement requires concrete implementation evidence and explicit confirmation. Ordinary `no_design_change` is a separate zero-write result valid only when no draft or invalidated ready authority awaits cleanup. An `implemented` record remains provenance and never changes status.
...
Models remain authority for confirmed business facts and the current falsifiable structural model. A confirmed EventStorming iteration increments each semantically changed `model_revision`, updates `last_changed_by` to those minutes, and integrates only current conclusions owned by that context. Iteration diagrams and cross-context scenario flow stay in the minutes instead of being copied into every Model.
...
| `event-storming/<event-storming-slug>.md` | EventStorming; Guard only for `ready -> implemented` | One iteration's confirmed scope and current structural model, integrated business-success diagrams with Role/fact-to-Capability Command edges, decisions and reasons, affected Models, assumptions, Hotspots, and status or correction lineage |
| `tactical-design/<tactical-design-slug>.md` | Tactical Design; Guard only for `ready -> implemented` | One Design Delta's domain-object thesis, state authority, semantic flow, necessity proof, minimal collaboration sequences, non-goals, and discretion; a reconciled ready record additionally contains implementation evidence, final claims, Architecture dispositions, and status or invalidation lineage |
...
Before Tactical Design reconciliation, write only its coherent common `draft` sections and unchecked README entry; do not pre-generate final claims, Architecture dispositions, or evidence placeholders. Authorized Codify exploration may change code but never DDD authority. After Tactical Design reconciles concrete implementation evidence, it revises that same draft with evidence and final claim disposition. If the user confirms the reconciled candidate, stage the exact `ready` transition, README, affected BC Architecture projections, and relevant ADR closure as one consistency set. Perform no Architecture write only when every claim is `iteration-only`; a projected `remove` may delete the optional file when its final current row is retired. Tactical Design never changes canonical Models; a business-meaning correction returns to EventStorming.

When newer ready EventStorming authority or concrete implementation evidence under the same ready authority invalidates an unimplemented ready Tactical Design, Codify and Guard stop at Tactical Design until the stale record is replaced or retired. A replacement ready write includes the old `ready -> superseded` lineage and complete Architecture reprojection; each changed surviving Architecture points `last_changed_by` to the replacement ready Tactical Design. If no replacement Design Delta remains, a retirement write points `superseded_by` to the surviving ready EventStorming minutes and removes every Architecture row sourced from the stale claims. Under unchanged EventStorming authority, that link names the authority now sufficient for realization and does not claim changed business facts; the write also requires concrete implementation evidence and explicit confirmation. It preserves unrelated rows and Sources byte-for-byte; when the file survives, its incremented revision points `last_changed_by` to the just-superseded record solely as retirement provenance, not current claim authority. Delete the optional file when no rows remain.
...
- Use [event-storming.md](event-storming.md) for one EventStorming iteration.
