...
| `tactical-design/<tactical-design-slug>.md` | Tactical Design; Guard only for `ready -> implemented` | One Design Delta, typed Model-to-responsibility collaboration sequences, ownership, changed Interfaces, claims, exhaustive Architecture dispositions, non-goals, discretion, and status or invalidation lineage |
...
Before Tactical Design confirmation, write only its `draft` record and unchecked README entry. After confirmation, stage the exact `ready` transition, README, exhaustive claim dispositions, affected BC Architecture projections, and relevant ADR closure as one consistency set. Perform no Architecture write only when every claim is `iteration-only`; a projected `remove` may delete the optional file when its final current row is retired. Tactical Design never changes canonical Models; a business-meaning correction returns to EventStorming.
