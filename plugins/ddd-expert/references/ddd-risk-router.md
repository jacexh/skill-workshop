...
- **Decision:** command writes use Domain Repositories; product reads use QueryRepository/read facades grouped by read-model family. The presence of QueryRepository names is not proof; shared infrastructure implementation does not prove CQRS separation. Classify caller semantics, DTO/read-model family, write-side repository methods, and adapter overlap.
