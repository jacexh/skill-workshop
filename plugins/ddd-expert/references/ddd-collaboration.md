...
- **[House Rule]** Strategic discovery may identify that an occurrence needs a local reaction or published meaning. Tactical object design records an actual Domain Event only on the object behavior that production code must emit or consume. Provider delivery attempts, lease changes, retry notifications, worker status, and log records remain execution observations.
...
- **[House Rule]** When an actual Domain Event or Published Fact Contract requires a next intent, record the producing or consuming object behavior and the required domain result. Choose handlers, transport, delivery, and recovery only through accepted design authority.
