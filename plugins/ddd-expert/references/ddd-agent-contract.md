...
Application command-side ports are high-risk deviations. Before creating one, answer this decision card in order:
...
8. Only after all above are rejected -> high-risk Application command-side port with the rejection reason and residual risk in the Architecture Gate.
...
- For a **manual polling/reconciliation runner** (`*_drain.go`, scheduler/recovery loop, timer/ticker goroutine, `fx.Lifecycle` background loop, or repeated backlog/outbox polling) → read `ddd-golang-taskqueue.md` and `ddd-golang-runtime.md`, then classify the observed role before accepting the file placement. If it is a scheduled trigger, use a periodic task contract and processor where the project has adopted taskqueue. If it is shared worker/scheduler lifecycle, place lifecycle/config/shutdown in runtime infrastructure. If it is Application coordination, keep the business orchestration in a named Application service and keep the cadence/runtime loop outside the bounded-context root unless a written runtime exception justifies the manual runner.
...
| Multi-aggregate write justification | Skipping the deviation review produces broken consistency boundaries | "Why does this need to write multiple aggregates in one transaction? Can it be split via Domain Events / Integration Messages / a named Application coordination service?" |
...
| Existing repository convention conflicts | Local architecture tests, project docs, or established imports may conflict with this guide; silently following the local workaround hides the decision | "This repository convention conflicts with the guide: <specific test/doc/import rule>. Should I update the convention, use the guide's default, or document an exception?" |
...
### 3.1 Repository Convention / Conflict Check

For Go DDD/business work, inspect the repository's local architecture tests, project standards, and adopted component imports before choosing a local implementation shape. If they conflict with this guide, report the conflict in the Architecture Gate and stop for user direction unless the user has already given an explicit exception.

Use targeted inspection, not a repo-wide fishing expedition. Typical checks:

```bash
rg -n 'forbidden.*Import|architecture.*test|go-jimu/components/(fsm|ddd/event|ddd/message|taskqueue|sloghelper)' .
rg -n '\.State ==|switch .*\.State|\.Status ==|switch .*\.Status' internal/*/application internal/*/domain
```

Add this block to the Architecture Gate when such a conflict could affect the implementation:

```text
Repository convention / conflict check:
...
- Resolution: proceed / stop for user decision / high-risk deviation
```
...
1. **Create technology-shaped ports.** Do not add `Cacher`, `RedisStore`, `RedisClient`, `MysqlReader`, `LockClient`, `TxManager`, `UnitOfWork`, `TransactionalEventPublisher`, or `BrokerPublisher` as Domain / Application interfaces. Compose those clients and transaction mechanics inside Infrastructure behind an existing `Repository` / `QueryRepository` / semantic port. (core §3.4, modeling §0.2; `ddd-golang-infrastructure.md`)
...
3. **Put generated proto / `pb.Message` into Domain.** Domain methods, value objects, repository interfaces, and domain events must use Domain-owned types. Convert `Proto ↔ Domain` at the Application or Infrastructure boundary. (core §5.7, `ddd-golang-scaffold.md §0.4`, `ddd-golang-application.md §0.7`)
...
7. **Write multiple aggregates in one transaction "because the ORM makes it easy".** The default is one aggregate per transaction. Multi-aggregate transactions are high-risk deviations and require the deviation gate in core §3.2. `xorm.Session` / `gorm.Tx` convenience is not a justification.
...
16. **Invent local substitutes for the adopted Go component libraries.** In Go DDD code using this guide's project-default stack, do not define project-local `DomainEvent`, `EventBus`, `EventDispatcher`, `MessagePublisher`, `MessageSubscriber`, `MessageRunner`, `FailurePolicy`, `TaskType`, `TaskQueue`, `TaskWorker`, `TaskSchemaRegistry`, `PeriodicTask`, `PeriodicTaskScheduler`, `StateMachine`, `LoggerHelper`, `ConfigLoader`, or similar equivalents for concerns already covered by `ddd-golang.md`'s default library table. Use the adopted library interfaces directly (`github.com/go-jimu/components/ddd/event`, `github.com/go-jimu/components/ddd/message`, `github.com/go-jimu/contrib/message/kafka`, `github.com/go-jimu/components/taskqueue`, `github.com/go-jimu/contrib/taskqueue/asynq`, `github.com/go-jimu/components/fsm`, `github.com/go-jimu/components/sloghelper`, `github.com/go-jimu/components/config`, etc.) unless existing repository code or explicit user direction establishes an exception. (`ddd-golang.md`, `ddd-golang-domain.md`, `ddd-golang-events-messages.md`, `ddd-golang-taskqueue.md`)
    - **FSM threshold bypass**: When a Go aggregate has 5+ business states plus guards, multiple actors/actions, an explicit transition table/diagram, or transitions that affect capacity/resources/idempotency/retry/archive/billing/permission outcomes, use `github.com/go-jimu/components/fsm` if the repository has adopted this guide's Go stack. If local architecture tests or project docs forbid the component, report the conflict through `ddd-golang-domain.md §0.5` instead of silently writing a local transition table.
    - **FSM state polymorphism bypass**: Do not use go-jimu FSM as only a transition table while keeping state-specific business behavior in Aggregate/Application `switch state` branches. Current State methods own state-specific behavior; Aggregate methods delegate to current State and then advance through private `fsm.Transit` plumbing. Do not reimplement Transit, use `HasTransition` as the primary permission check, or expose StateMachine operations as public business APIs.
...
- [ ] **Repository convention conflicts surfaced**: local architecture tests, project standards, and adopted component conventions that affect this change were inspected; conflicts with this guide appear in the Architecture Gate instead of being silently bypassed.
- [ ] **Aggregate decision API**: Application/handlers/processors do not make business decisions by branching on aggregate fields. Any remaining field reads are mechanical mapping, read-model assembly, persistence mapping, logging enrichment, or an explicit Architecture Gate exception.
...
- [ ] **Local helper discipline**: same-package helpers were reused before new private helpers were added; every single-call helper names a durable rule/contract or boundary normalization, otherwise it was inlined; no `*Local` duplicate helper was introduced.
...
| Multi-aggregate transaction deviation gate | [`ddd-core.md`](ddd-core.md) §3.2 |
...
| Go Application / generated RPC / logging | [`ddd-golang-application.md`](ddd-golang-application.md) |
| Go CQRS read side | [`ddd-golang-cqrs.md`](ddd-golang-cqrs.md) |
| Go persistence / adapters | [`ddd-golang-infrastructure.md`](ddd-golang-infrastructure.md), [`database.md`](database.md) |
