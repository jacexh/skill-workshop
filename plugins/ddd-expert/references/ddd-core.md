---
name: ddd-core
description: Compact DDD + Clean Architecture baseline for backend services. Use when a phase skill needs layer ownership, dependency direction, repositories, Domain Events, Integration Messages, CQRS, cross-context contracts, or architecture conformance rules.
---
...
Do not merge write-side Repository and read-side QueryRepository responsibilities merely because one adapter or table can serve both. CQRS read-shaped method inventory must inspect every read-shaped method on write repositories or shared adapters before claiming the read/write split is correct.
...
- Should the boundary be merged into one Aggregate Root, or should the collaboration be eventual-consistency through Domain Events/process/recovery?
...
- Repository normally exposes only `Get` and `Save`; other methods start as Repository/API smells.
