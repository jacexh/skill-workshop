...
common references. Apply each conditional stack and placement rule only when
the accepted implementation contains that code surface.
...
[`ddd-collaboration.md`](ddd-collaboration.md).
...
Create conditional directories only when the accepted implementation contains
that code surface.
...
`setup()` in Domain with I/O-free guards/actions. Persist business state and
version rather than opaque runtime snapshots.
...
When a post-commit Domain Event flow is accepted, add the event buffer and
dispatcher conditionally: capture/drain inside the transaction, dispatch only
after `execute` resolves, and handle the dispatcher result through the ordinary
Application error and logging convention. Otherwise omit all event machinery.
...
Application logs only valuable business facts; the outer execution boundary owns
...
extracts identity, and delegates once. Kafka provider mechanics stay out.

An accepted `transport/task-processor` decodes one `Job<unknown>` and delegates
once. BullMQ and Redis provider mechanics stay out.
...
provider configuration, credentials, and timeouts stay outer. Preserve the first
external error with `cause`; never leak driver text to Transport.

## 8. Events and Integration Messages
...
and shutdown. Use a stable message key for the accepted ordering
scope and propagate message ID, contract kind/version, occurred time,
correlation, and W3C trace context. Runtime commits offsets at its accepted
provider boundary after successful handler completion.
...
QueueEvents, Redis, concurrency, attempts/backoff, Job
...
a bound, closes providers/Kysely, then telemetry. Runtime reports unexpected
loop exit to the process entrypoint.

Use Pino structured records with one execution-completion owner:

- Fastify owns HTTP/Connect duration and public outcome;
- Kafka Runtime owns attempt and commit outcome;
- BullMQ Runtime owns task duration, attempt, and terminal outcome;
...
- Application tests prove the selected lifecycle and mapping with real handlers
  and focused fakes; accepted events also prove capture, persistence, and
  dispatch order.
...
- Accepted Kafka and BullMQ paths run real Testcontainers smoke/integration
  tests for registration, delivery, commit, malformed work, scheduling, trace
  propagation, and shutdown drain.
...
- [`ddd-collaboration.md`](ddd-collaboration.md): published APIs, Domain Events, and Integration Messages.
