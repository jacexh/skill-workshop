...
Event Timeline Reconciliation: for lifecycle reviews, compare `spec/design fact | Domain Event type | handler/reconciler/process manager` before saying event coverage is satisfied. Recovery reachability proof means the handler/reconciler/process manager must be wired in production/runtime, not only named in design or tests. A repository transaction is not a substitute for a missing same-BC Domain Event reaction; if a fact needs repeated same-context reaction and no event/reaction/process owner exists, return to `domain-modeling` for fact/owner ambiguity or `design` for accepted-model placement.
