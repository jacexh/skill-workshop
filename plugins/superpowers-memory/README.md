...
- `superpowers-memory:ingest` forces source review, owner routing, targeted Core Query Coverage, exclusion checks, index regeneration, and verification before commit; `update` and `rebuild` remain compatibility aliases for incremental and bootstrap/full-refresh modes.
...
- `hook-runtime.js verify` checks stale path references, shape violations, ADR integrity, readiness warnings, SSOT duplication, retrieval cost, split candidates, architecture coverage gaps, and commit readiness.
